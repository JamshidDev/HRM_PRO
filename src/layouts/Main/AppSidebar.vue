<script setup>
  ...
  const rememberSidebar = () => {
    ...
    let store = localStorage.getItem('sidebarStore')
      ? JSON.parse(localStorage.getItem('sidebarStore'))
      : null
    if (store) {
    ...
    <template v-if="item?.children?.length > 0">
      <div class="multiple__item">
        ...
        <div
          class="items__panel"
          :style="{
            height: panelPath === item.path ? item.children.length * 32 + 2 + 'px' : '0px'
          }"
        >
          <div
