<script setup>
  ...
  const deployProject = () => {
    loading.value = true
    axios
      .get('http://localhost:9000/api/deploy')
      .then((res) => {
  ...
  const showVideo = () => {
    const callback = examVideoStore.showCanvas
      ? examVideoStore.stopCanvasRender
      : examVideoStore.startCanvasDraw
    callback()
  }
</script>

<template>
  <div class="app-header-bar relative  flex justify-between items-center ">
    <div
    ...
    <div class="flex items-center z-10">
      <div
        @click="controlBtn"
        class="mobile-control-btn bg-dark flex justify-center items-center cursor-pointer w-[32px] h-[32px] rounded mr-2"
      >
