...
<template>
  <n-modal v-model:show="store.previewVisible" draggable style="width: 100%; height: 96vh">
    <div
      class="ui-preview-window w-full h-full grid grid-cols-12 max-w-[1400px] bg-surface-ground rounded-md overflow-hidden"
    >
      <n-spin
      ...
      >
        <div class="flex w-full h-full" :class="[store.panelVisible && 'preview-panel-active']">
          <div
            class="preview-menu h-full bg-[url('/wing.png')] border-r border-surface-line bg-surface-ground flex flex-col px-4 pt-6 z-10"
          >
            <div class="relative w-full">
              <div
                v-if="showImgController"
                class="absolute top-1/2 translate-y-[-50%] right-1 cursor-pointer"
              >
              ...
              <div
                v-if="showImgController"
                class="absolute top-1/2 translate-y-[-50%] left-1 cursor-pointer"
              >
                ...
                >
                  ...
                  <template v-else>
                    <n-carousel-item class="hover:scale-90 transition-all rounded-lg">
                      <img class="w-full" :src="Utils.noAvailableImage" alt="worker image" />
                    </n-carousel-item>
                  </template>
                </n-carousel>
                <div
            ...
            <template v-if="store.workerPreview">
              <div
                ...
                class="cursor-pointer w-full p-2 mb-2 flex items-center gap-2 rounded-md border border-surface/20 hover:bg-primary hover:text-white transition-all hover:scale-90"
