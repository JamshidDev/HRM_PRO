  ...
  <div>
    <SectionHeader :title="$t('medPage.name')" :icon="MedicalExaminationIcon" icon-chip>
      ...
      <div v-if="store.workerPreview?.meds?.length" class="flex flex-col gap-6">
        ...
        >
          <div class="md:col-span-1 bg-success/5 rounded-3xl p-4 relative overflow-hidden">
    ...
    <div v-if="store.workerPreview?.meds?.length" class="flex flex-col gap-6 mt-4">
      <div
        v-for="(item, idx) in store.workerPreview?.meds"
        :key="idx"
      ...
      >
        ...
        />
        <InfoBox surface="section" :icon="ShieldCheckmark20Filled" :label="$t('workerView.med.validityPeriod')">
          <template #value>
            <div>{{ Utils.timeOnlyDate(item.to) }}</div>
            <div class="text-xs text-textColor3 font-normal">{{ monthsLeft(item) }}</div>
          </template>
        </InfoBox>
        <InfoBox surface="section" :icon="CheckmarkCircle20Filled" :label="$t('workerView.med.conclusion')">
          <template #value>
