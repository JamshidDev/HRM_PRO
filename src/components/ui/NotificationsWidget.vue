...
const onScrollEv = ()=>{
  ...
  if((element.scrollTop+element.offsetHeight+20 > element.scrollHeight) && store.userUnreadNotificationsCount>store.userUnreadNotifications.length) {
...
watch(()=>store.viewingNotification, (v)=>{
  if(v){
  ...
  <n-popover class="!p-0 ![--n-border-radius:20px]" placement="bottom-start" trigger="click" @update:show="onShow" :show="showPopover">
    ...
    <div class="w-[300px] h-[300px] cursor-pointer overflow-hidden grid-rows-[auto_1fr_auto] grid">
      ...
      <n-spin class="overflow-hidden" :show="store.userUnreadNotificationsLoading">
        <NoDataPicture v-if="!store.userNotificationsLoading && store.userUnreadNotifications.length===0" class="!m-0 [&_svg]:!size-[120px] [&_.text-textColor2]:text-sm [&_.text-textColor3]:text-xs" />
        ...
        <div ref="scrollContainer" class="w-full h-full overflow-y-auto px-1" @scroll="onScrollEv">
          <template v-for="item in store.userUnreadNotifications" :key="item.id">
            ...
            >
                ...
                <div class="grid grid-cols-[auto_1fr] gap-1">
                  <n-tooltip trigger="hover">
                    <template #trigger>
                      <div>
                      <component
                        :class="cn('size-5 p-0.5 rounded-full inline-block shrink-0',
                        item.data.alert===notificationTypes.info.value && 'bg-info/20 text-info',
                        item.data.alert===notificationTypes.success.value && 'bg-success/20 text-success',
                        item.data.alert===notificationTypes.warning.value && 'bg-warning/20 text-warning',
                        item.data.alert===notificationTypes.error.value && 'bg-danger/20 text-danger')"
                  ...
                  {{ item?.data?.title }}
                    ...
                    <span class="text-xs text-textColor3">{{dayjs(item.created_at).format('MMM DD, HH:mm')}}</span>
                  </div>
                </div>
              ...
              <div class="flex justify-between items-center mt-1 pb-1">
              </div>
            </div>
          </template>
        </div>
...
<!--          @click="goViewAll"-->
  ...
  <UIModal v-model:visible="showViewModal" :width="700">
    <template #header-title>
      ...
      <span class="text-sm text-textColor3">{{dayjs(store.viewingNotification.created_at).format('MMM DD, HH:mm')}}</span>
    </template>
    <div class="grid grid-rows-[1fr_auto] h-[500px] relative" v-if="store.viewingNotification?.data?.message">
      <UIEditorViewer :html="store.viewingNotification?.data?.message" />

      <component :is="notificationTypes[store.viewingNotification?.data?.alert]?.icon" class="size-20 absolute right-0 bottom-10 text-textColor3/60" />
      <component :is="notificationTypes[store.viewingNotification?.data?.alert]?.icon" class="size-10 absolute right-0 bottom-30 text-textColor3/60" />
      <component :is="notificationTypes[store.viewingNotification?.data?.alert]?.icon" class="size-8 absolute right-20 bottom-12 text-textColor3/60" />
      <component :is="notificationTypes[store.viewingNotification?.data?.alert]?.icon" class="size-6 absolute right-18 bottom-28 text-textColor3/60" />

      <div class="flex items-center justify-between">
        <NotificationBadge v-if="store.viewingNotification.data.alert" :alert="store.viewingNotification.data.alert" />
        <p class="text-sm text-textColor3">{{$t('content.read')}}: <b>{{dayjs(store?.viewingNotification?.read_at || undefined).format('MMM DD, HH:mm')}}</b></p>
      </div>
    </div>
