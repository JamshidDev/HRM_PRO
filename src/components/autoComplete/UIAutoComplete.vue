...
<template>
  <div class="ui__autocomplete flex flex-col">
    ...
    <div
      ...
      v-if="visible"
      class="ui__autocomplete-panel shadow bg-surface-section border border-surface-line w-full top-[65px] absolute z-999 h-[60px] p-2 rounded-xl"
    >
      <n-spin
        class="w-full h-full flex justify-center items-center"
        :show="store.pinLoading"
        size="small"
      >
        <template v-if="store.worker">
          <n-collapse-transition :show="Boolean(store.worker)">
            <div
              @click="onSelectEv(store.worker)"
              class="w-full hover:bg-blue-50 cursor-pointer transition-all rounded-xl"
            >
