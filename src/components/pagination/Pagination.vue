<template>
  <div class="grid xl:grid-cols-2 lg:grid-cols-2 grid-cols-1   gap-4 py-4 w-full">
    <div class="flex xl:justify-start lg:justify-start md:justify-start justify-center text-textColor2">
      <span class="font-normal"><span class="font-semibold  text-primary">{{total }}</span> {{$t('content.fromTa')}} <span class="font-semibold text-primary ml-4">{{(page -1)*pageSize}} - {{page*pageSize}} </span>{{$t('content.untilShow')}}</span>
    </div>
