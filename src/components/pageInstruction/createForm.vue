...
<template>
  <div style="height: calc(100vh - 140px)" class="overflow-y-auto overflow-x-hidden mb-[20px]">
    <n-form class="px-2" ref="formRef" :rules="validationRules.common" :model="store.payload">
      <n-form-item
        @paste="onPaste"
        :show-label="false"
        path="photos"
      ...
      >
        <div
          v-if="!store.payload.photos.length"
          class="text-secondary/20 page_instruction_image flex justify-center flex-col items-center"
        >
          <NoImage class="w-[200px]" />
          <p class="text-secondary/40 text-xl">{{ $t('content.noImage') }}</p>
        </div>
        <n-carousel
          v-model:current-index="store.imgCarouselIdx"
          v-else
          :draggable="store.payload.photos.length > 1"
          draggable
          autoplay
          show-arrow
          :show-dots="false"
        ...
        <div class="absolute left-[5px] bottom-[5px]">
          <n-button-group size="tiny">
            ...
            <n-button
              type="error"
              v-if="store.payload.photos.length"
              @click="store.deleteImage"
              :loading="store.imageLoading"
            >
              {{ $t('content.delete') }}
              <template #icon>
                <n-icon>
