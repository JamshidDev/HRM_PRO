...
<template>
  <div class="flex flex-col gap-2">
    ...
    <n-input-group>
      ...
      <n-button @click="onOpenAttach" type="primary">
        <template #icon>
          <n-icon size="26">
            <AttachText20Regular />
          </n-icon>
    ...
    <n-collapse-transition
      :show="store.fileShow"
      class="bg-surface-section p-2 rounded-sm overflow-hidden"
    >
      <template v-for="(item, idx) in store.fileList" :key="idx">
        <template v-if="item?.file">
          <div class="flex justify-between py-1 px-2 border-b border-surface-line cursor-pointer">
            ...
            <span @click="onDownload(item.file)" class="w-[220px] truncate hover:text-primary">{{
              item.original_name
            }}</span>
          </div>
        </template>
