  ...
  actions: {
    ...
    _userList(infinite) {
      ...
      $ApiService.userService
        ._index({ params })
        .then((res) => {
          let selectedData = Array.isArray(this.payload.users)
            ? this.userList.filter((v) => this.payload.users.includes(v.id))
            : []
          ...
          this.userList = infinite
            ? Array.from(
                new Map([...this.userList, ...data, ...selectedData].map((v) => [v.id, v])).values()
              )
            : Array.from(new Map([...data, ...selectedData].map((v) => [v.id, v])).values())
    ...
    onScrollUserList(e) {
      ...
      if (
        ...
        this.userListTotal > this.userList.length
      ) {
        this.userListParams.page += 1
        this._userList(true)
      }
