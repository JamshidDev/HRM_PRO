...
export const useExamAttemptStore = defineStore('examAttemptStore', {
  ...
  actions: {
    _downloadResult(params) {
      this.loading = true
      $ApiService.workerExamService
        ._downloadResult({ id: this.elementId, params })
        .then((res) => {
      ...
      $ApiService.workerExamService
        ...
        .then((res) => {
          ...
          localStorage.setItem(
            'exam_data',
            JSON.stringify({ ...data, [worker_exam_details.id]: active_token })
          )
          router.push({
            name: 'solve_exam',
            params: {
    ...
    _continue_attempt(callback) {
      ...
      this.loading = true
      $ApiService.workerExamService
        ._continue_exam({ id: this.elementId, token: this.exam_token })
        .then((res) => {
    ...
    _finish_attempt(callback) {
      $ApiService.workerExamService
        ._finish_exam({ id: this.elementId, token: this.exam_token })
        .then((res) => {
          let data = localStorage.getItem('exam_data')
          data = data ? JSON.parse(data) : {}
    ...
    _get_attempt() {
      this.loading = true
      $ApiService.workerExamService
        ._get_attempt({ id: this.elementId })
        .then((res) => {
