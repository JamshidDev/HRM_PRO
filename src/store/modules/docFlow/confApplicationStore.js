...
export const useConfApplicationStore = defineStore('confApplicationStore', {
  ...
  actions: {
    ...
    _confDelete() {
      this.loading = true
      $ApiService.applicationService
        ._deleteWorkerApplication({ id: this.elementId })
        .finally(() => {
          this.loading = false
          this._index()
        })
    ...
    _myPositions(callback) {
      ...
      $ApiService.applicationService
        ._myPositions()
        .then((res) => {
          this.myPositionList = res.data.data.map((v) => ({
            ...v,
            position: Utils.combineFullName(v.worker),
            name: v.post_short_name
          }))
    ...
    _vacationWorker(id) {
      ...
      $ApiService.applicationService
        ._vacationWorker({ params: { organizations: id } })
        .then((res) => {
    ...
    _onActiveVacation(id) {
      ...
      $ApiService.applicationService
        ._workerVacation({ data: { worker_positions: [id] } })
        .then((res) => {
    ...
    _update() {
      ...
      $ApiService.applicationService
        ._updateWorkerApplication({ data, id: this.elementId })
        .then((res) => {
          this.visible = false
          this._index()
        })
