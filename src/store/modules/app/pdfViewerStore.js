...
export const usePdfViewerStore = defineStore('pdfViewerStore', {
  ...
  actions: {
    ...
    _generateLink(data) {
      ...
      $ApiService.documentService
        ._generateLink({ params: data })
        .then((res) => {
          this.linkVisible = true
          this.link = Utils.convertFromUrlToQuery(
            res.data.data.url,
            Utils.viewerStatus.signatureDocument
          )
        })
        .finally(() => {
    ...
    _checkSignature(params) {
      ...
      $ApiService.documentService
        ._signature({ params, data: { status: 'check' } })
        .then((res) => {
