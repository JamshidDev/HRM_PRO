...
export const useComponentStore = defineStore('componentStore', {
  ...
  actions: {
    createDepartmentFetcher(key) {
      ...
      return async (params = {}, infinity = false) => {
        ...
        try {
          const {
            data: { data }
          } = await $ApiService.componentService._departmentByOrganizations({ params })
    ...
    createPositionFetcher(key) {
      ...
      return async (params = {}, infinity = false) => {
        ...
        try {
          const {
            data: { data }
          } = await $ApiService.positionService._filterIndex({ params })
    ...
    _lmsLearningCenter(callback) {
      ...
      $ApiService.eduPlanService
        ._learningCenter({ params: { page: 1, per_page: 1000 } })
        .then((res) => {
    ...
    _searchWorkerByPin(queryPin) {
      ...
      this.workerPinList = []
      $ApiService.workerService
        ._checkWorker({ params: { pin: queryPin } })
        .then((res) => {
          if (!res.data.error) {
            let data = res.data.data
            this.workerPinList = [
    ...
    _examCategory() {
      ...
      $ApiService.componentService
        ._examCategory({ params: { ...this.params } })
        .then((res) => {
    ...
    _docExample() {
      ...
      $ApiService.componentService
        ._docExample({ params: this.params })
        .then((res) => {
    ...
    _structures() {
      ...
      $ApiService.componentService
        ._structure({ params: this.structureParams })
        .then((res) => {
    ...
    _allStructures() {
      ...
      $ApiService.componentService
        ._allStructure({ params: this.structureParams })
        .then((res) => {
    ...
    _adContractType(id) {
      ...
      $ApiService.componentService
        ._contractAddition({ params: { contract_type: id } })
        .then((res) => {
    ...
    _workerApplicationEnums() {
      ...
      $ApiService.componentService
        ...
        .then((res) => {
          this.workerApplicationTypes = res.data.data.application_types.map((v) => ({
            ...v,
            name: v.id + ' - ' + v.name
          }))
          this.educationTypes = res.data.data.education_types
    ...
    _turnstileTerminal() {
      ...
      $ApiService.turnstileTerminalService
        ._index({ params: { ...this.params } })
        .then((res) => {
