...
const {t} = i18n.global
export const useTurnstileScheduleStore = defineStore('turnstileScheduleStore', {
    state: () => ({
        params:{
            page:1,
            per_page:10,
    ...
    getters:{
        ...
        calculateFactTime:(state)=>(workerIndex)=>{
            if(state.workerList[workerIndex].days.length === 0) return 0
            const factTimeMinute = state.workerList[workerIndex].days.reduce((sum, day) => {
                return sum + (day?.factWorkTime || 0)
            }, 0)

            return Math.floor(factTimeMinute/60)
        },
    },
    ...
    actions: {
        ...
        _allWorkers(){
            ...
            $ApiService.workerScheduleService._factIndex({params}).then((res) => {
                ...
                const formatSchedule = (day)=>{
                    let factClass = null
                    const factWorkTime = Number(day.fact_daytime) + Number(day.fact_evening_time)
                    const dayWorkTime = Number(day.daily_minutes)
                    factClass = factWorkTime === 0? 'bg-danger/40':factWorkTime>dayWorkTime? 'bg-success/80':'bg-warning/80'
                    return {
                        ...
                        firstIn:Utils.timeOnlyHour(day.first_in),
                        lastOut:Utils.timeOnlyHour(day.last_out),
                        factWorkTime:Number(day.fact_daytime) + Number(day.fact_evening_time),
                        factClass:factClass,
                    }
                }

                const formatData = (array=[], count=0)=>{
                    ...
                    for(let day=1; day<=count; day++){
                        ...
                        if(!items){
                            ...
                            results.push({day, ...formatSchedule(items[0])})
                        }
                        else{
                            ...
                            results.push({
                                day,
                                date:items[0].date,
                                children:items.map(formatSchedule),

                            })
                            if(items.length > 2) console.error('Length greater than max', items.length)
                        }
                    }
                    return results
                }
                ...
                this.workerList = res.data.data.data.map((v, index)=> {
                    ...
                    const count = this.dayOfMonth.length
                    const data = formatData(v.schedules, count)
