  ...
  >
    ...
    >
      <div class="col-span-12 flex justify-between mb-4 items-center">
        ...
        <n-switch
          @change="onChangeGrand"
          size="large"
          v-model:value="store.grandPayload.is_turnstile"
        />
      </div>
      <n-collapse-transition class="col-span-12" :show="store.grandPayload.is_turnstile">
        <div class="col-span-12 p-2 bg-warning/10 rounded-lg leading-[1.3] text-secondary mb-4">
          {{ $t('shiftType.form.grandLabel') }}
        </div>
        <n-form-item
          class="col-span-12"
          :label="$t(`shiftType.form.startWorkTime`)"
          path="start_minute"
        ...
        <n-form-item
          class="col-span-12"
          :label="$t(`shiftType.form.endWorkTime`)"
          path="end_minute"
          :rule-path="validationRules.rulesNames.requiredNumberField"
        >
          <n-input-number
        ...
        <n-form-item class="col-span-12" :label="$t(`content.comment`)">
          <n-input :rows="2" type="textarea" v-model:value="store.grandPayload.comment" />
        </n-form-item>
      ...
      <div class="col-span-12 flex justify-between mb-4 items-center">
        ...
        <n-switch
          @change="onChange"
          size="large"
          v-model:value="store.grandPayload.grandToReport"
        />
      </div>
      ...
      <div class="grid grid-cols-2 gap-2 mt-4 col-span-12">
        ...
        <n-button @click="onSubmit" :loading="store.savingLoading" type="primary">
          {{ $t('content.save') }}
        </n-button>
      </div>
    </n-form>
