        ...
        >
          ...
          <tr v-for="(item, idx) in store.list" :key="idx">
            ...
            <td class="!text-center">
              <n-button @click="onPreviewEv(item)" :type="item.buttonType" ghost circle size="small">
                <span class="text-[14px] font-bold">{{item.workTime}}</span>
              </n-button>
            </td>
            <td>
              <UIBadge v-if="item?.firstEnterTime" :show-icon="false" :label="item?.firstEnterTime"></UIBadge>
            </td>
            <td>
              <UIBadge v-if="item?.firstExistTime" :type="Utils.colorTypes.error" :show-icon="false" :label="item?.firstExistTime"></UIBadge></td>
            <td>
