      ...
      <div class="flex mx-auto">
        <div class="w-[200px] pb-4">
          <template v-if="!item.direction">
            ...
            <p class="text-xs text-right"><span class="text-primary bg-primary/10 rounded-lg px-2 font-semibold">{{item.device_name}}</span></p>
          </template>
        ...
        <div class="w-[200px] pb-4">
          <template v-if="item.direction">
            ...
            <p class="text-xs"><span class="text-primary bg-primary/10 rounded-lg px-2 font-semibold">{{item.device_name}}</span></p>
          </template>
        </div>
      </div>
    </template>
    <p v-if="store.selectedList.length === 0" class="text-secondary text-center !mt-10">{{$t('content.no-data')}}</p>
  </div>
</template>