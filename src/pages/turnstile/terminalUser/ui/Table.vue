      ...
      <n-table class="mt-5 w-full table-fixed" :single-line="false" size="small">
        <thead>
          <tr>
            ...
            <th class="text-center! max-w-[200px] w-[200px]">
              {{ $t('turnstile.terminalUser.terminals') }}
            </th>
        ...
        <tbody>
          <tr v-for="(item, idx) in store.list" :key="idx">
            ...
            <td class="text-center!">
              <n-popover
                v-if="item.terminals_count"
                :show="!!(store.instanceData && item.worker.id === store.elementId)"
                ...
                <div>
                  <div class="h-[150px] max-h-[150px] overflow-y-auto">
                    <div v-for="(option, idx) in store.instanceData?.terminals" :key="option.id">
                      ...
                      <n-checkbox
                        @click="() => toggleDetachTerminals(option.id)"
                        :checked="!store.payload.detachTerminals.includes(option.id)"
                      >
                ...
                <template #trigger>
                  <div class="group relative inline-block">
                    <n-spin :show="store.instanceDataLoading">
                      ...
                      <div
                        class="opacity-0 group-hover:opacity-100 transition-all absolute left-0 top-0 right-0 bottom-0"
                      >
