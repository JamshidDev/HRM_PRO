<script setup>
  ...
  const deviceRenderLabel = (option) => {
    if (option?.option) {
      ...
      return [h('div', { class: 'font-medium text-gray-500' }, option.name)]
    }
    return [
      h('div', { class: 'flex flex-col' }, [
        h('div', { class: 'text-xs font-medium text-gray-500' }, option.name),
  ...
  <n-spin :show="store.terminalsLoading">
    <div style="height: calc(100vh - 120px)" class="overflow-y-auto">
      ...
      >
        ...
        >
          <div
            class="h-[250px] max-h-[250px] p-2 border border-surface-line border-dashed w-full rounded-md relative"
          >
            ...
            <div class="absolute bottom-[10px] right-[10px]">
              <n-button
                :disabled="typeof store.payload.photo_index === 'number'"
                @click="openCropper"
                type="primary"
                size="tiny"
              >
        ...
        >
          ...
          >
            <template #action>
              <n-checkbox
                v-model:checked="store.payload.allTerminalsChecked"
                @update:checked="onChangeCheckAll"
                >{{ $t('content.selectAll') }}</n-checkbox
              >
            </template>
          </n-select>
        </n-form-item>
