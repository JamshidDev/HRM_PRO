    ...
    <n-tooltip placement="top" trigger="hover">
      <template #trigger>
        <n-icon
          ...
          :class="[worker.canRecognize ? 'text-success' : 'text-warning']"
        >
      ...
      <span v-else>
        {{ $t('shiftType.form.turnstileActive') }} {{ worker.turnstileStartTime }}/{{
          worker.turnstileEndTime
        }}
      </span>
    </n-tooltip>
    <div class="flex flex-col w-[calc(100%-50px)] mr-1">
      <span class="leading-[1.2] text-textColor2 font-medium text-xs line-clamp-1">{{
        worker?.fullName
      }}</span>
