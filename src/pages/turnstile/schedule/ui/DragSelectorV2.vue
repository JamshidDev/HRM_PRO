<script setup>
  ...
  const emit = defineEmits(['selection-start', 'selection-change', 'selection-end'])
  ...
  const updateSelectedItems = () => {
    ...
    emit(
      'selection-change',
      [...newSelected].map((v) => JSON.parse(v))
    )
  }

  const calculateScrollSpeed = (distance) => {
    return (
      props.minScrollSpeed +
      (distance / props.scrollZone) * (props.maxScrollSpeed - props.minScrollSpeed)
    )
  }
  ...
  const performAutoScroll = () => {
    ...
    const canScrollDown = container.scrollTop < maxScrollTop - 1
