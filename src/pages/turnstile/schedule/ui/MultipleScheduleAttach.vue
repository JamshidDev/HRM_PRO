...
<template>
  ...
  >
      ...
      <n-checkbox-group v-model:value="store.selectedWorkers">
        <div class="w-full h-[calc(100vh-360px)] overflow-y-auto px-2">
          <template v-for="item in store.attachWorkers">
            <n-checkbox :class="[item.id === store.selectedWorkerId && 'opacity-40']" :disabled="item.id === store.selectedWorkerId" class="!w-full !items-center flex px-4 py-1 bg-surface-ground/30 rounded-lg border border-surface-line mb-2" size="large" :value="item.id">
              <UIUser
                  class="!w-full"
                  :hide-tooltip="true"
                  :short="false"
    ...
    <div class="flex justify-end px-4 gap-4 py-4">
      <n-button @click="store.multipleAttachVisible=false" type="error" secondary>{{$t('content.cancel')}}</n-button>
