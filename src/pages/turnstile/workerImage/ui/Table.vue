<script setup>
import {NoDataPicture, UIPagination, UIMenuButton, UIUser, UIBadge, UIImageViewer} from "@/components/index.js"
...
const getStatusType = (status) => {
  switch(status) {
    case 1: return Utils.colorTypes.warning
    case 2: return Utils.colorTypes.success
    case 3: return Utils.colorTypes.warning
    default: return Utils.colorTypes.error
  }
}
    ...
    <div class="w-full overflow-x-auto"  v-if="store.list.length>0">
      ...
      >
        ...
        <tr v-for="(item, idx) in store.list" :key="idx">
          ...
          <td>
            <UIBadge :show-icon="false" :label="getStatusLabel(item.status)" :type="getStatusType(item.status)" />
          </td>
          <td>
            <UIBadge 
                :show-icon="false" 
          ...
          <td>
            <span :class="[item.status === 3? 'text-primary' : 'text-danger']"  class="text-xs leading-[1.2] line-clamp-2">{{ item.error }}</span>
          </td>
          <td>
            <UIMenuButton
                :show-delete="false"
