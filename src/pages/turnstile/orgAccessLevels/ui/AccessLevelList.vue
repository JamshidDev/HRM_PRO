<script setup>
  ...
  const onSearchEv = () => {
    ...
    if (!Boolean(query)) {
      ...
      store.accessLevels = store.originAccessLevels.filter((v) =>
...
<template>
  <div class="flex flex-col gap-2">
    ...
    <div class="h-[calc(100vh-244px)] overflow-auto">
      <div class="grid grid-cols-12 gap-2 overflow-auto p-1">
        <div
          v-for="(item, idx) in store.accessLevels"
          :key="idx"
        ...
        >
          <n-checkbox
            :checked="(() => !!store.payload.access_levels.find((i) => i === item.id))()"
          />
          <div>
    ...
    <div
      class="flex gap-4 justify-between items-center bg-surface-section py-2 px-2 border border-surface-line rounded-lg"
    >
      <div class="flex">
        <n-button
          @click="onCancelOrg"
          ...
          type="warning"
          secondary
          v-if="store.payload.organization.length > 0"
        >
          <span class="line-clamp-1 max-w-[300px]! overflow-ellipsis">{{
            store.payload.organization[0].name
          }}</span>
          <template #icon>
            <DismissCircle32Filled />
          </template>
      ...
      <div class="flex gap-2">
        <n-button class="md:hidden" type="error" @click="cancelChanges">
          {{ $t('content.cancel') }}
        </n-button>
        <n-button
