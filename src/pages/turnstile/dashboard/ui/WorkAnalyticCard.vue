...
<template>
  <div
    class="w-full !h-full bg-surface-section px-3 py-2 rounded-lg border border-surface-line hover-effect-card"
  >
    <div v-if="dashboardStore.workerDataWithSchedule" class="grid grid-cols-12">
      ...
      <div class="col-span-12 flex items-center justify-between overflow-hidden">
        <span
          class="shrink-0 xl:text-nowrap lg:text-wrap text-nowrap xl:text-sm min-w-[120px] text-xs leading-[1.2] text-secondary flex items-center gap-1"
        >
        ...
        <span
          class="shrink-0 xl:text-nowrap xl:text-sm text-xs text-primary bg-primary/4 px-2 py-1 lowercase text-nowrap s"
          >{{ dashboardStore.workerDataWithSchedule?.total_workers || 0 }}
          {{ $t('content.person') }}</span
        >
      </div>

      <div class="col-span-12 flex items-center justify-between overflow-hidden my-1">
        <span
          class="shrink-0 xl:text-nowrap lg:text-wrap text-nowrap xl:text-sm min-w-[120px] leading-[1.2] text-xs text-secondary flex items-center gap-1"
        >
        ...
        >
        <div class="flex-1 border-t border-dashed border-surface-line mx-2"></div>
        <span class="shrink-0 text-nowrap text-success bg-success/4 px-2 py-1 lowercase"
          >{{ dashboardStore.workerDataWithSchedule?.workers_with_schedule || 0 }}
          {{ $t('content.person') }}</span
        >
      </div>
      ...
      <div class="col-span-12 flex items-center justify-between overflow-hidden">
        <p
          class="shrink-0 xl:text-nowrap lg:text-wrap text-nowrap xl:text-sm text-xs min-w-[80px] leading-[1.2] text-secondary flex items-center gap-1"
        >
          <n-icon size="20" class="text-danger bg-danger/4">
            <DismissCircle24Filled />
          </n-icon>
          {{ $t('turnStileDashboard.work_durations.workerCountWithoutSchedule') }}
        </p>
        <div class="flex-1 border-t border-dashed border-surface-line mx-2"></div>
        <span
          class="shrink-0 xl:text-nowrap lg:text-wrap text-nowrap xl:text-sm text-xs min-w-[90px] leading-[1.2] text-danger bg-danger/4 px-2 py-1 lowercase"
          >{{ dashboardStore.workerDataWithSchedule?.workers_without_schedule || 0 }}
          {{ $t('content.person') }}</span
        >
      </div>
      ...
      <template v-if="dashboardStore.workerDataWithSchedule?.schedule_statistics">
        <div
          class="col-span-12 grid grid-cols-12 border border-surface-line border-dashed p-2 rounded-lg mt-1 bg-surface-ground/40"
        >
          <div
            v-for="(item, idx) in dashboardStore.workerDataWithSchedule?.schedule_statistics"
            :key="idx"
            class="mb-1 col-span-12 flex items-center justify-between overflow-hidden"
          >
            <p
              class="shrink-0 xl:text-nowrap lg:text-wrap text-nowrap xl:text-sm text-xs min-w-[80px] leading-[1.2] text-secondary flex items-center gap-1"
            >
            ...
            <div class="flex-1 border-t border-dashed border-surface-line mx-2"></div>
            <span
              class="shrink-0 xl:text-nowrap lg:text-wrap text-nowrap xl:text-sm text-xs min-w-[90px] leading-[1.2] text-primary bg-primary/4 px-2 py-1 lowercase"
              >{{ item?.worker_count || 0 }} {{ $t('content.person') }}</span
            >
          </div>
        </div>
      </template>
