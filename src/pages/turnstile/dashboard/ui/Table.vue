...
const renderComponent = (componentName, value, componentProps = {}) => {
  switch (componentName) {
    ...
    case 'UIBadge':
      const badgeProps = typeof componentProps === 'string' 
        ? { type: componentProps }
        : componentProps
      return h(UIBadge, {
        label: value,
  ...
  <div class="w-full">
    <div class="w-full overflow-y-auto h-[calc(100vh-256px)]">
      ...
      >
        <thead>
        <tr>
          <th v-if="showNumbering" class="text-center! min-w-[40px] w-[40px]">{{$t('content.number')}}</th>
        ...
        <template v-for="(row, index) in data" :key="index">
          <tr >
            <td v-if="showNumbering" class="text-center">
              <span class="text-center text-[12px] text-gray-600 block">{{ (page - 1) * perPage + index + 1 }}</span>
            </td>
            <template v-for="(item, idx) in columns" :key="idx">
              <td>
                ...
                <component 
                  v-if="item.component"
                  :is="renderComponent(item.component, item.formatValue ? item.formatValue(getNestedValue(row, item.key)) : getNestedValue(row, item.key), typeof item.componentProps === 'function' ? item.componentProps(getNestedValue(row, item.key)) : item.componentProps)"
                />
                <!-- Aks holda oddiy text ko'rsatamiz -->
                <template v-else>
                  {{ item.formatValue ? item.formatValue(getNestedValue(row, item.key)) : getNestedValue(row, item.key) }}
                </template>
              </td>
            </template>
          </tr>
