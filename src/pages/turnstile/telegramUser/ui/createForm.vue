<script setup>
  ...
  const onSearchEv = () => {
    ...
    if (!Boolean(query)) {
      ...
      const filtered = store.originDeviceList.filter((v) =>
  ...
  watchEffect(() => {
    store.inputParam.selectAll =
      store.deviceList.length > 0 ? store.deviceList.length === store.payload.devices.length : false
  })
  onMounted(() => {
          ...
          <template #action>
            <n-checkbox @update:checked="onCheck" v-model:checked="store.inputParam.selectAll">
              <span class="text-secondary text-xs"
                >{{ $t('content.selectAll') }} - ({{ store.deviceList.length }})</span
              >
            </n-checkbox>
          </template>
        </n-select>
