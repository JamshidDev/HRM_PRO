...
<template>
  <div style="height: calc(100vh - 120px)" class="overflow-y-auto">
    ...
    >
      <n-form-item
        :label="$t(`turnstile.terminalPage.building`)"
        path="building_id"
      ...
      <n-form-item
        :label="$t(`turnstile.terminalPage.name`)"
        path="name"
      ...
      <n-form-item
        :label="$t(`turnstile.terminalPage.name_ru`)"
        path="name_ru"
        rule-path="requiredStringField"
      >
      ...
      <n-form-item :label="$t(`content.url`)" path="url" rule-path="requiredStringField">
        <n-input type="text" v-model:value="store.payload.url" />
      </n-form-item>
      <n-form-item :label="$t(`content.ip`)" path="ip_address" rule-path="requiredStringField">
        <n-input type="text" v-model:value="store.payload.ip_address" />
      </n-form-item>
    </n-form>
  </div>
