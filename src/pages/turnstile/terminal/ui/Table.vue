      ...
      <n-table class="mt-3 w-full table-fixed" :single-line="false" size="small">
        <thead>
          <tr>
            ...
            <th class="text-center! min-w-[150px] w-[150px]">
              {{ $t('turnstile.terminalPage.name') }}
            </th>
            <th class="text-center! min-w-[150px] w-[150px]">
              {{ $t('turnstile.terminalPage.name_ru') }}
            </th>
            <th class="text-center! min-w-[150px] w-[150px]">
              {{ $t('turnstile.terminalPage.name_en') }}
            </th>
            <th class="text-center! min-w-[100px] w-[100px]">
              {{ $t('turnstile.terminalPage.building') }}
            </th>
        ...
        <tbody>
          <tr v-for="(item, idx) in store.list" :key="idx">
            ...
            <td>
              <UIMenuButton :show-edit="true" :data="item" @select-ev="onSelect" />
            </td>
          </tr>
        </tbody>
      </n-table>
