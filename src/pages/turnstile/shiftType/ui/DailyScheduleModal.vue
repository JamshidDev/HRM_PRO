        ...
        <div class="w-full overflow-y-auto h-[600px] pr-2 mt-4">
          <template v-if="store.notScheduleWorkerList.length > 0">
            <n-checkbox-group v-model:value="store.selectedWorkers">
              <n-table :single-line="false" size="small" class="sticky-table-header">
                <thead>
                  <tr>
                  ...
                  <tr v-for="(item, idx) in store.notScheduleWorkerList" :key="idx">
                    <td>
                      <span class="text-center block">{{
                        (store.notScheduleParams.page - 1) * store.notScheduleParams.per_page +
                        idx +
                        1
                      }}</span>
                    </td>
        ...
        <div class="flex justify-between gap-4 mt-10">
          <div class="flex gap-2">
            <n-button type="primary" secondary
              >{{ $t('content.selected') }}
              <span class="font-bold ml-2">{{ store.selectedWorkers.length }}</span></n-button
            >
          ...
          <div class="flex gap-4">
            <n-button type="error" secondary @click="store.notScheduleVisible = false">{{
              $t('content.cancel')
            }}</n-button>
