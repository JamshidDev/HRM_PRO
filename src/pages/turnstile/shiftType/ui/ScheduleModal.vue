...
import {Utils} from "@utils"
  ...
  watchEffect(() => {
    const month =
      store.uiParams.month?.toString()?.length > 1
        ? store.uiParams.month
  ...
  >
    <div>
        ...
        <div class="w-full overflow-auto relative h-[460px] mt-4">
          ...
          <template v-if="currentScheduleList?.[0]?.length > 0">
            ...
            >
              ...
              <div
                class="border-r border-l-[0] border-b border-t-0 border-surface-line p-2 w-[320px] min-w-[260px] h-[50px] border sticky left-[60px] bg-surface-section flex-shrink-0 z-[5] flex flex-col"
              >
                <UINSelect
                  clearable
              ...
              <template v-for="(day, dayIndex) in worker">
                ...
                >
                  ...
                  <template v-else-if="worker?.[dayIndex].isWorkDay">
                    ...
                    >
                      {{ Math.floor(worker?.[dayIndex]?.workTime / 60) }}
                    </div>
                  </template>
                  <template v-else>
                    <div
          ...
          <template v-else>
            <div
              class="w-full text-center h-[300px] sticky left-0 flex justify-center items-center text-secondary"
            >
              {{ $t('content.no-data') }}
            </div>
          </template>
      ...
      <div class="flex justify-end gap-4 pt-6">
        <n-button @click="store.scheduleVisible = false" type="error">{{
          $t('content.cancel')
        }}</n-button>
