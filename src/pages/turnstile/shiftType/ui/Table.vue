          ...
          <template v-for="(item, idx) in store.list" :key="item.id">
            <div
               ...
               rounded-tr-lg gap-2 px-2 pt-[10px] items-center
                w-full"
            >
              <div class="w-[40px] text-center text-sm text-secondary">{{ (store.params.page - 1) * store.params.per_page + idx + 1 }}</div>
              ...
              <div class="flex flex-col text-xs text-secondary md:w-[260px] w-full">
                ...
                <p class="leading-[1.2] line-clamp-1 w-full"
                   >
                  {{ $t('shiftType.form.groupCount') }}: <span
                    class="font-semibold text-textColor0">{{item.groups}}</span></p>
                <p class="leading-[1.2] line-clamp-1">{{ $t('shiftType.form.workerCount') }}: <span
                    class="font-semibold text-textColor0">{{item.workers}}</span></p>
              </div>
              <div class="flex flex-col text-xs md:w-[260px] w-full text-secondary">
                <n-button @click="onGroups(item)" class="max-w-[200px]"> <template #icon><Eye16Filled/></template> {{$t('shiftType.form.showGroup')}}</n-button>
              </div>
              <div class="flex md:w-[300px]! !w-full md:justify-end">
                <n-button @click="onOpenModal(item)" type="primary">{{$t('shiftType.form.createGroup')}}</n-button>
              </div>
            </div>
