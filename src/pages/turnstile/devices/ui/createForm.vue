  ...
  formRef.value?.validate((error) => {
    if (!error) {
      ...
      const data =store.isAdmin? {
        ...
        organization_id:store.payload.organization_id.map(v=>v.id).toString() || undefined,
      ...
      >
        ...
        <template v-if="store.isAdmin">
          <n-form-item class="col-span-6" :label="$t('devices.device_code')" path="device_code">
            <n-input
                v-model:value="store.payload.device_code"
            />
          </n-form-item>
          ...
          <n-form-item class="col-span-6" :label="$t('devices.upload_workers')" path="upload_workers">
            <n-switch
          ...
          <div class="col-span-12">
            <div class="grid grid-cols-12 gap-x-4 bg-surface-ground p-2 rounded-lg border border-surface-line">
              <h3 class="col-span-12 text-center mb-4 text-secondary">{{$t('devices.contractTitle')}}</h3>
              <n-form-item class="col-span-6" :label="$t('documentPage.form.contractNumber')" path="contract_number">
                <n-input
                    v-model:value="store.payload.contract_number"
                />
              </n-form-item>
              <n-form-item class="col-span-6" :label="$t('documentPage.form.contractDate')" path="contract_date">
                <n-date-picker
                    class="w-full"
              ...
              <n-form-item class="col-span-12" :label="$t('content.price')" path="price">
                <n-input
                    v-model:value="store.payload.price"
                />
              </n-form-item>
            </div>
