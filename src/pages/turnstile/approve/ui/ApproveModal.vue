  ...
  >
    <n-spin :show="store.showLoading">
      <div class="grid gap-2 grid-cols-12 w-full">
        ...
        <div class="col-span-12 rounded-xl bg-surface-ground p-2 h-[132px] overflow-y-auto border border-surface-line">
          ...
          <div class="flex flex-wrap gap-2 mt-1">
            <template v-for="item in store.approve.accessLevels" :key="item.id">
              <span class="bg-primary text-white rounded-xl px-2 py-1 text-xs">{{item.name}}</span>
            </template>
          </div>
        </div>
        <div class="col-span-12 bg-surface-ground p-2 rounded-xl h-[calc(100vh-500px)] overflow-y-scroll border border-surface-line">
    ...
    <div class="grid grid-cols-12 gap-4 mt-4">
      <n-button @click="onSubmit('rejected')" :loading="store.saveLoading" class="col-span-6" type="error">{{$t('content.reject')}}</n-button>
      <n-button @click="onSubmit('approved')" :loading="store.saveLoading" class="col-span-6" type="primary">{{$t('content.confirm')}}</n-button>
    </div>
