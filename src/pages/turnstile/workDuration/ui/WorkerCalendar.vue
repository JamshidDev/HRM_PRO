...
<template>
  <n-spin class="worker-calendar" :show="store.instanceLoading">
    <n-tabs
      :value="store.dayLogParams.building ? 2 : 1"
      :tab-style="{ display: 'none' }"
      :pane-style="{ padding: 0 }"
      animated
    >
      <n-tab-pane :name="1">
        <n-calendar @panel-change="onPanelChange" :value="null">
          <template #default="{ year, month, date }">
            ...
            >
              <template
                v-for="(data, idx) in store.instanceData[store.dateToKey(year, month, date)].data"
                :key="idx"
              >
                  ...
                  <n-tooltip trigger="hover">
                    <template #trigger>
                      <div
                        @click="
                          viewDayLogs(
                            store.instanceData[store.dateToKey(year, month, date)].date,
                            data.building
                          )
                        "
                        class="h-[40px] w-[40px] transition-all bg-primary hover:bg-primary/80 rounded-full flex justify-center items-center text-surface-section font-bold text-xl cursor-pointer"
                      >
      ...
      <n-tab-pane :name="2">
        <div class="flex justify-between p-2 pl-0 border-b border-surface-line">
          <div class="flex gap-2 items-center">
            ...
            <p class="text-lg" v-if="store.dayLogParams.date">
              {{ Utils.timeOnlyDate(store.dayLogParams.date) }}
            </p>
          </div>
        ...
        <div class="min-h-[500px] max-h-[600px] overflow-y-auto py-3 flex flex-col log-timeline">
          <template v-for="(item, idx) in store.dayLogList" :key="idx">
            <div class="flex gap-3 justify-center h-[50px]">
              ...
              <div class="h-full relative">
                ...
                <div
                  v-if="idx !== store.dayLogList.length - 1"
                  class="action-line"
                  :class="{ 'leave-line': item.event_type, 'enter-line': !item.event_type }"
                />
              </div>
              <div class="w-[300px] translate-y-[-5px]">
                <template v-if="!item.event_type">
