<script setup>
  ...
  const isHideColumn = computed(() =>
    ['late_workers', 'early_leave_workers'].includes(store.previewParams.type)
  )
              ...
              <n-table :single-line="false" size="small">
                <thead>
                  <tr>
                    ...
                    <th v-if="isHideColumn" class="min-w-[130px] w-[130px] !text-center">
                      {{ $t('content.date') }}
                    </th>
                    <th v-if="isHideColumn" class="min-w-[110px] w-[110px] !text-center">
                      {{ $t('content.hour') }}
                    </th>
                    <th v-if="isHideColumn" class="min-w-[100px] w-[100px]">
                      {{ $t('hcEvent.form.direction') }}
                    </th>
                    <th v-if="isHideColumn" class="min-w-[100px] w-[300px]">
                      {{ $t('hcEvent.form.device') }}
                    </th>
                    <th v-if="isHideColumn" class="min-w-[100px] w-[160px]">
                      {{ $t('hcEvent.form.device') }}
                    </th>
                    <th v-if="isHideColumn" class="min-w-[100px] w-[120px] !text-center">
                      {{ $t('hcEvent.form.mask_status') }}
                    </th>
                    <th v-if="isHideColumn" class="min-w-[100px] w-[100px] !text-center">
                      {{ $t('hcEvent.form.temperature') }}
                    </th>

                    <th v-if="!isHideColumn && !isShowMinuteColumn" class="min-w-[100px] w-[500px]">
                      {{ $t('content.position') }}
                    </th>
                    <th v-if="!isHideColumn && !isShowMinuteColumn" class="min-w-[100px] w-[300px]">
                      {{ $t('content.department') }}
                    </th>

                    <th v-if="isShowMinuteColumn" class="min-w-[100px] w-[300px]">
                      {{ $t('hcEvent.form.workTime') }}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(item, idx) in store.previewList" :key="idx">
                    <td>
                      <span class="text-center block">{{
                        (store.previewParams.page - 1) * store.previewParams.per_page + idx + 1
                      }}</span>
                    </td>
                    <td>
                      <div>
