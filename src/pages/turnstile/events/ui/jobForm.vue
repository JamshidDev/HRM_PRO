    ...
    <div>
      ...
      >
        <div class="border border-surface-line rounded-lg border-dashed px-4 bg-surface-ground/40">
          ...
          <n-form-item path="f_date" :rule-path="validationRules.rulesNames.requiredNumberField" :show-label="false" :label="$t(`content.deadline`)">
              ...
              <div style="height:calc(100vh - 500px)" class="overflow-y-auto">
                ...
                >
                  ...
                  <tr v-for="(item, idx) in store.jobList" :key="idx">
                    <td class="w-[20px] max-w-[20px]"><span class="text-center text-[12px] text-gray-600 block">{{ (store.jobParams.page - 1) * store.jobParams.per_page + idx + 1 }}</span></td>
                    <td>
