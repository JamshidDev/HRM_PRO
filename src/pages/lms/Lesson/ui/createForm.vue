<script setup>
  ...
  const onChangeTeacher = () => {
    ...
    if (store.payload.teacher_id) {
      store.subjectList = store.teacherList
        .filter((v) => v.id === Number(store.payload.teacher_id))?.[0]
        .subjects.map((x) => ({ ...x, position: ' ' }))
    }
  }
...
<template>
  <n-form ref="formRef" :rules="validationRules.common" :model="store.payload">
    <div class="grid grid-cols-12 gap-x-4">
      ...
      <n-form-item
        class="col-span-6"
        :label="$t(`lessonPage.form.edu_plan_id`)"
        path="edu_plan_id"
        rule-path="requiredNumberField"
      >
        <n-select
      ...
      <n-form-item
        class="col-span-6"
        :label="$t(`lessonPage.form.teacher_id`)"
        path="teacher_id"
        rule-path="requiredNumberField"
      >
        <n-select
      ...
      <n-form-item
        class="col-span-6"
        :label="$t(`lessonPage.form.subject_id`)"
        path="subject_id"
        rule-path="requiredNumberField"
      >
        <n-select
      ...
      <n-form-item
        class="col-span-6"
        :label="$t(`lessonPage.form.lesson_date`)"
        path="lesson_date"
        rule-path="requiredNumberField"
      >
        <n-date-picker
      ...
      <n-form-item
        class="col-span-3"
        :label="$t(`lessonPage.form.start_time`)"
        path="start_time"
        rule-path="requiredStringField"
      >
        <n-input v-mask="'##:##'" type="text" v-model:value="store.payload.start_time" />
      </n-form-item>
      <n-form-item
        class="col-span-3"
        :label="$t(`lessonPage.form.end_time`)"
        path="end_time"
        rule-path="requiredStringField"
      >
        <n-input v-mask="'##:##'" type="text" v-model:value="store.payload.end_time" />
      </n-form-item>
    </div>
