...
<template>
  <div class="grid grid-cols-12 gap-x-4 gap-y-2 w-full">
    ...
    <div class="col-span-6 border border-surface-line rounded-lg p-2 bg-surface-ground/30">
      <p class="font-semibold leading-[1.2]">
        {{ store.selectedLesson?.start_time }} -:- {{ store.selectedLesson?.end_time }}
      </p>
    ...
    <div
      class="col-span-12 border bg-surface-ground/20 border-surface-line rounded-lg border-dashed p-4 mt-10"
    >
      <div class="grid grid-cols-12 gap-2">
        <div class="col-span-12">
          <p class="text-secondary text-center">
            {{
              $t(
                store.selectedLesson?.exam
                  ? 'lessonPage.detail.attachedLesson'
                  : 'lessonPage.detail.attachmentLesson'
              )
            }}
          </p>
        </div>
        <div class="col-span-12" v-if="store.selectedLesson?.exam">
          <div
            class="w-full px-2 py-2 text-center border border-surface-line rounded-lg text-primary flex gap-2 items-center justify-between"
          >
            ...
            >
            <n-icon
              @click="store._detachExam(store.selectedLesson?.exam?.id)"
