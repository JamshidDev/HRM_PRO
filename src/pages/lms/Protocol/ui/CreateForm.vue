...
<template>
  <UIModal :width="'600px'" v-model:visible="store.protocol.visible" :title="$t('protocol.name')">
    <div
      class="bg-warning/4 px-4 py-2 rounded-xl border border-warning text-secondary leading-[1.2]"
    >
    ...
    <n-spin :show="store.protocol.loading">
      <div class="w-full h-[400px] overflow-y-auto px-2 mt-4">
        <div
          @click="onSelect(0)"
          class="mt-2 border border-dashed border-success/20 cursor-pointer hover:bg-success/10 flex gap-2 bg-success/4 mb-2 px-2 py-2 rounded-lg"
        >
          <div class="w-[40px] flex justify-center items-center">
            <div
              :class="0 === store.payload.protocol_id ? 'bg-success' : 'bg-primary'"
              class="w-[32px] h-[32px] flex justify-center items-center text-white rounded-2xl"
            >
              <n-icon size="24">
        ...
        <template v-for="item in store.protocol.list">
          <div
            @click="onSelect(item)"
            class="border border-surface-line cursor-pointer hover:bg-surface-ground/90 flex gap-2 bg-surface-ground/50 mb-2 px-2 py-1 rounded-lg"
          >
            <div class="w-[40px] flex justify-center items-center">
              <div
                :class="item.id === store.payload.protocol_id ? 'bg-success' : 'bg-primary'"
                class="w-[32px] h-[32px] flex justify-center items-center text-white rounded-2xl"
              >
                <n-icon size="24">
    ...
    <n-button
      @click="onContinue"
      :disabled="store.payload.protocol_id === null"
      class="!w-full"
      type="primary"
      >{{ $t('content.continue') }}</n-button
    >
  </UIModal>
</template>
