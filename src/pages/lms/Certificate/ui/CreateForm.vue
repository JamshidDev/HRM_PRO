    ...
    <template #default>
      ...
      >
        ...
        <n-spin :show="store.workerLoading" class="col-span-12">
          <div
            class="border border-surface-line h-[calc(100vh-260px)] mb-6 rounded-xl overflow-y-auto relative"
          >
            <div
              class="col-span-12 bg-surface-section grid grid-cols-12 gap-x-6 sticky top-0 z-10 mb-2 border-b border-surface-line"
            >
              <div class="col-span-4 px-2 py-1 font-medium">{{ $t('content.worker') }}</div>
              <div class="col-span-4 px-2 py-1 font-medium">
                {{ $t('lmsCertificate.form.start_exam_result') }}
              </div>
              <div class="col-span-4 px-2 py-1 font-medium">
                {{ $t('lmsCertificate.form.end_exam_result') }}
              </div>
            </div>
            <template v-for="(worker, idx) of store.payload.workers" :key="worker.id">
              <div
                class="mx-2 grid grid-cols-12 items-center gap-x-4 mb-2 bg-surface-ground/60 py-1 px-2 rounded-xl border border-surface-line"
              >
