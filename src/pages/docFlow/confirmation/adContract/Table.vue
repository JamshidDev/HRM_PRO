      ...
      <n-table class="mt-10" :single-line="false" size="small">
        ...
        <tbody>
          <tr v-for="(item, idx) in store.list" :key="idx">
            ...
            <td>
              <span
                @click="onOpenFile(item?.contract_additional.id, item.id)"
                class="hover:text-primary hover:underline cursor-pointer"
                >{{ item?.contract_additional?.type.name }}</span
              >
            </td>
            <td>
              <UIUser
            ...
            <td>
              <div class="flex justify-center">
                <n-button class="font-medium" round type="error" size="tiny">{{
                  item?.contract_additional?.number
                }}</n-button>
              </div>
            </td>
