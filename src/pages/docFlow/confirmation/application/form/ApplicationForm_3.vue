...
<template>
  <div class="grid grid-cols-12 gap-x-4">
    <div class="col-span-12">
      <div
        class="flex flex-col border-b border-surface-line mb-4 border-dashed transition-all"
        :class="store.vacationShow && 'border-t border-r border-l p-2 rounded-md'"
      >
        <div class="flex justify-between items-center w-full mb-4 cursor-pointer">
          <span @click="store.vacationShow = !store.vacationShow" class="text-primary">{{
            $t(`applicationPage.lastVacationTitle`)
          }}</span>
          <n-icon size="18" class="text-primary">
            <ChevronRight12Regular
              class="transition-all"
              :class="store.vacationShow && 'rotate-90'"
            />
          </n-icon>
        </div>
