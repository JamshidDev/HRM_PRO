...
<template>
  ...
  >
    <div class="col-span-12">
      <template
        v-if="vacation && vacation?.period_from && store.vacationId === store.payload.worker"
      >
        <n-collapse-transition v-show="Boolean(vacation)">
          ...
          >
            ...
            <div>
              <div
                class="font-medium"
                :class="[vacation.rest_day >= 0 ? 'text-success' : 'text-danger']"
              >
                {{ vacation.rest_day }}
              </div>
    ...
    <div class="col-span-12 md:col-span-6 lg:col-span-3">
      <n-form-item :label="$t(`commandPage.form_45.to`)" path="to">
        <n-date-picker
          ...
          type="date"
          :format="useAppSetting.datePicketFormat"
          :disabled="!store.form_45.from && !store.form_45.to"
          :is-date-disabled="(v) => Utils.disablePasteDate(v, store.form_45.from)"
        />
      </n-form-item>
    </div>
    <div class="col-span-12 md:col-span-6 lg:col-span-3">
      <n-form-item :label="$t(`commandPage.form_45.work_day`)" path="work_day">
        <n-date-picker
          ...
          type="date"
          :format="useAppSetting.datePicketFormat"
          :disabled="!store.form_45.to && !store.form_45.work_day"
          :is-date-disabled="(v) => Utils.disablePasteDate(v, store.form_45.to)"
        />
      </n-form-item>
    </div>
  </n-form>
