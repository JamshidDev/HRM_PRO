<script setup>
  ...
  const showPension = computed(() =>
    Boolean(store.form_34.command_additional.find((v) => v === typeConfig.pensionCount))
  )
  const showPensionCoefficient = computed(() =>
    Boolean(store.form_34.command_additional.find((v) => v === typeConfig.pensionCoefficient))
  )
  const showCompensation = computed(() =>
    Boolean(store.form_34.command_additional.find((v) => v === typeConfig.compensation))
  )
  const showSalaryWithholding = computed(() =>
    Boolean(store.form_34.command_additional.find((v) => v === typeConfig.salaryWithHolding))
  )
    ...
    (newValue, oldValue) => {
      ...
      store._additionalData({ type: paramType }, (res) => {
        ...
        if (type === typeConfig.pensionCount) {
          ...
          store.form_34.compensation.period1 = data.period1
            ? new Date(data.period1).getTime()
            : null
          store.form_34.compensation.period2 = data.period2
            ? new Date(data.period2).getTime()
            : null
          ...
          store.form_34.salary_withholding.period1 = data.period1
            ? new Date(data.period1).getTime()
            : null
          store.form_34.salary_withholding.period2 = data.period2
            ? new Date(data.period2).getTime()
            : null
  ...
  const selectOptions = computed(() => {
    return componentStore.deleteCommandEnum.map((v) => ({
      ...v,
      disabled: store.form_34.command_additional.includes(pairs[v.id])
    }))
  })
  ...
  const options_34 = [
    ...
    {
      id: 100,
      name: t('documentPage.command.form.warning_six')
    }
  ]
  ...
  // 100 => this special number for backend which detects the last item of options
  const disableReasonInput = computed(() => store.form_34.reasonId !== 100)
  ...
  const onSelectReason = (v) => {
    store.form_34.reasonId = v.id
    store.form_34.reason = v.id === 100 ? null : v.name
  }
...
<template>
  ...
  >
    ...
    <n-spin :show="pensionCountLoading" class="col-span-12">
      <div
        v-if="showPension"
        class="grid grid-cols-12 gap-x-4 gap-y-2 p-2 border border-dashed rounded-lg border-surface-line bg-surface-section/80"
      >
    ...
    <n-spin :show="compensationLoading" class="col-span-12">
      ...
      >
        <div class="col-span-12 text-xs">
          <n-button type="primary" secondary size="tiny">{{
            componentStore.deleteCommandEnum?.[3]?.name
          }}</n-button>
        </div>
    ...
    <n-spin :show="salaryWithholdingLoading" class="col-span-12">
      <div
        v-if="showSalaryWithholding"
        class="grid grid-cols-12 gap-x-4 gap-y-2 p-2 border border-dashed rounded-lg border-surface-line bg-surface-section/80"
      >
        <div class="col-span-12 text-xs">
          <n-button type="primary" secondary size="tiny">{{
            componentStore.deleteCommandEnum?.[2]?.name
          }}</n-button>
        </div>
        <LastVacationItem ref="lastVacationRef" />
        <div class="col-span-12 flex justify-center mb-1">
