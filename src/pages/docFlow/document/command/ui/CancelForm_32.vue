<script setup>
      ...
      if (!error) {
        data = {
          ...
          command_additional: {
            ...(isMedCommand.value
              ? {
                  med_date: Utils.timeToZone(store.form_32.warning_date),
                  med_number: store.form_32.warning_number || null
                }
              : {
                  warning_date: isShowWarningItem.value
                    ? Utils.timeToZone(store.form_32.warning_date)
                    : undefined,
                  warning_number: isShowWarningItem.value
                    ? store.form_32.warning_number || null
                    : undefined
                }),
  ...
  const showPension = computed(() =>
    Boolean(store.form_32.command_additional.find((v) => v === typeConfig.pensionCount))
  )
  const showPensionCoefficient = computed(() =>
    Boolean(store.form_32.command_additional.find((v) => v === typeConfig.pensionCoefficient))
  )
  const showCompensation = computed(() =>
    Boolean(store.form_32.command_additional.find((v) => v === typeConfig.compensation))
  )
  const showSalaryWithholding = computed(() =>
    Boolean(store.form_32.command_additional.find((v) => v === typeConfig.salaryWithHolding))
  )
    ...
    (newValue, oldValue) => {
      ...
      store._additionalData({ type: paramType }, (res) => {
        ...
        if (type === typeConfig.pensionCount) {
          ...
          store.form_32.compensation.period1 = data.period1
            ? new Date(data.period1).getTime()
            : null
          store.form_32.compensation.period2 = data.period2
          ...
          store.form_32.salary_withholding.period1 = data.period1
            ? new Date(data.period1).getTime()
            : null
          store.form_32.salary_withholding.period2 = data.period2
            ? new Date(data.period2).getTime()
            : null
  ...
  const selectOptions = computed(() => {
    return componentStore.deleteCommandEnum.map((v) => ({
      ...v,
      disabled: store.form_32.command_additional.includes(pairs[v.id])
    }))
  })
