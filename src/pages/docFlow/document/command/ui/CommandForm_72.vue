...
const onSubmit = (mainData) => {
  const checkForm = store.workerData.every((v) => (v.reason !== null && v.fine_type !== null && (v.fine_type ===2? v.fine !== null : true )))
  if (checkForm) {
    const data = store.workerData.map(v => ({
      id: v.id,
      reason: v.reason,
      fine_type: v.fine_type,
      fine:v.fine_type ===2? v.fine : componentStore.fineTypes[v.fine_type-1]?.name ,
    }))
...
<template>
  <div v-for="(item, idx) in store.workerData" :key="idx"
    ...
    <div class="col-span-12 md:col-span-6 lg:col-span-4">
      <n-form-item
          :show-feedback="false"
          :label="$t(`documentPage.command.form.fine_type`)" path="fine_type">
        <n-select
    ...
    <div v-if="item.fine_type===2"
         class="col-span-12 md:col-span-6 lg:col-span-4">
      <n-form-item
          :show-feedback="false"
          :label="$t(`documentPage.command.form.fine`)" path="fine">
        <n-input
            class="w-full"
            type="text"
