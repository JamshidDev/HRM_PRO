<script setup>
    ...
    await formRef.value?.validate(async (error) => {
      if (!error) {
        ...
        data = {
          ...
          half_two_date:
            store.form_46.half_two_base === 2
              ? Utils.timeToZone(store.form_46.half_two_date)
              : undefined
        }
      } else {
        data = null
  ...
  watchEffect(() => {
    store.form_46.half_two_day =
      Number(store.form_46.result.all_day) - Number(store.form_46.half_one_day)
  })
