<script setup>
  import { Drag24Filled, DrawText24Regular, DismissCircle28Filled } from '@vicons/fluent'
  import validationRules from '@/utils/validationRules.js'
  import { useComponentStore } from '@/store/modules/index.js'
  import { useCommandFormStore } from './commandFormStore.js'
  ...
  // create — yaratish modali; edit — hujjat drawer'idagi «Ma'lumotlar» tabi.
  ...
  const singleSelectCommands = [
    32, 33, 34, 35, 36, 37, 38, 39, 44, 43, 45, 46, 47, 49, 48, 50, 51, 52, 53, 54, 74, 56, 57, 58
  ]
    ...
    new Promise((resolve) => {
      formRef.value?.validate(async (error) => {
        ...
        if (!error) {
          ...
          resolve(validate?.isValid ? { isValid: true, data: validate.data } : { isValid: false })
        } else {
  ...
  const onChangeCommandType = () => {
    ...
    if (store.payload.command_type === CANCEL_COMMAND_TYPE) {
      ...
      store.payload.cancel_command_id = null
      store.cancelCommandList = []
      store.cancelCommandParams.organization_id = store.payload.organization_id?.[0]?.id ?? null
      store.cancelCommandParams.page = 1
      store.cancelCommandParams.search = null
  ...
  // Modal kontenti ochilganda mount bo'ladi — shuning uchun immediate.
  watch(
    () => store.visible,
    (val) => {
      if (val && !isEdit.value) loadLists()
    },
    { immediate: true }
  )

  // Kelishuvchilar ro'yxati kech yuklansa, direktor/moliya chiqarib tashlangan variantlar qayta hisoblanadi.
  watch(
    () => componentStore.confirmationList,
    () => syncConfirmationEv()
  )

  onMounted(() => {
    if (isEdit.value) loadLists()
  })
...
<template>
  <div class="command-form-body" :class="{ 'is-readonly': readonly }">
    ...
    >
      <div
        v-if="isEdit"
        class="col-span-12 mb-1.5 text-xs font-semibold uppercase tracking-wide text-textColor3"
      >
        {{ $t('documentPage.command.dataTab.sectionMain') }}
      </div>
      <!--      Command header-->
      ...
      <div
        v-if="isEdit"
        class="col-span-12 mt-6 mb-1.5 text-xs font-semibold uppercase tracking-wide text-textColor3"
      >
        {{ $t('documentPage.command.dataTab.sectionWorkers') }}
      </div>
      <div class="col-span-12">
        <div
          class="grid grid-cols-12 gap-x-4 border border-surface-line border-dashed p-2 rounded-md bg-surface-ground"
      ...
      <div
        ...
        class="col-span-12 mt-6 mb-1.5 text-xs font-semibold uppercase tracking-wide text-textColor3"
      >
        {{ $t('documentPage.command.dataTab.sectionDetails') }}
      </div>
      <div class="col-span-12" :class="{ 'mt-4': !isEdit }">
      ...
      <div class="col-span-12 mb-6" :class="{ 'mt-6': !isEdit }">
        <div
          class="grid grid-cols-12 gap-x-4 border border-surface-line border-dashed p-2 rounded-md bg-surface-ground"
        >
...
<style scoped>
  /* Faqat ko'rish rejimi — maydonlar o'qiladi, lekin o'zgartirilmaydi. */
  .command-form-body.is-readonly :deep(form) {
    pointer-events: none;
  }
</style>
