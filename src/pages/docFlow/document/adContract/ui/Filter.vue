<script setup>
  ...
  const onHistory = () => {
    router.push({ path: AppPaths.Audit, query: { trigger_name: 'hr.contract-additional' } })
  }
</script>

<template>
  <UIPageFilter
    :add-permission="accStore.pn.hrContractAdditionalWrite"
    @onAdd="onAdd"
    @onSearch="onSearchEv"
  ...
  >
    ...
    <template #filterEnd>
      <n-button
        v-if="accStore.checkPermission(accStore.pn.hrContractAdditionalRead)"
