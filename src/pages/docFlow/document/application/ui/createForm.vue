...
<template>
  <n-form ref="formRef" :rules="validationRules.applicationPage" :model="store.payload">
    <n-tabs animated v-model:value="store.activeTab" class="hidden-tab-header" type="segment">
        ...
        <div class="grid grid-cols-12 gap-x-4">
          ...
          <div class="col-span-12">
            <n-form-item :label="$t(`applicationPage.form.from_date`)" path="from_date">
              <n-date-picker class="w-full" v-model:value="store.payload.from_date" type="date" />
            </n-form-item>
          </div>
      ...
      <n-tab-pane :name="store.tabList[1]">
        <div class="grid grid-cols-12 gap-x-4">
          <div class="col-span-12">
            <div
              class="w-full border border-dashed border-surface-line rounded-md bg-surface-ground min-h-[100px] p-2 flex justify-center items-center text-gray-400"
            >
              {{ store.applicationLink || $t('content.no-data') }}
            </div>
          </div>
          <div class="col-span-12 mt-6">
            <n-button style="width: 100%" @click="copyLink" type="primary">
              <template #icon>
                <Copy20Regular />
              </template>
              {{ $t('content.copy') }}
          ...
          <div class="col-span-12 mt-4 mb-6">
            <n-button @click="store.activeTab = 1" secondary style="width: 100%" type="error">
              <template #icon>
