<script setup>
    ...
    $ApiService.workerService
      ._userRole({
        params: {
          ...userRoleParams,
          organizations:
            store.payload.filter.organizations.map((i) => i.id).toString() || undefined,
  ...
  const renderAvatarOption = ({ option }) => {
    const isItemSelected = store.payload.all
      ? !store.payload.unCheck.includes(option.id)
      : store.payload.userIds.includes(option.id)

    return h(
      'div',
      {
        class:
          'flex items-center justify-between cursor-pointer transition-all hover:bg-info/20 rounded-lg',
        onClick: () => {
...
<template>
  <div class="relative w-full">
    ...
    <div
      v-if="store.payload.all"
      class="flex items-center gap-2 absolute top-0 left-0 bottom-0 pl-4"
    >
