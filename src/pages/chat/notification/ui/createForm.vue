...
<template>
  <n-spin :show="componentStore.enumLoading">
    <n-form :disabled="store.saveLoading" ref="formRef" :rules="validationRules.common" :model="store.payload" size="large">
      <div class="grid grid-cols-2 gap-x-4">
        ...
        <n-form-item :label="$t('content.type')" path="alert" rule-path="requiredStringField">
          <n-select v-model:show="showNotificationAlertSelectMenu" v-model:value="store.payload.alert"
                    :menuProps="{
                     class: 'p-2'
                  }"
        ...
        <n-form-item class="col-span-2" :label="$t('content.users')" path="userIds" :rule="[{
          trigger: ['input', 'blur-sm'],
          validator(){
                return (!store.payload.all && !store.payload.userIds.length) ? new Error(t(`rules.requiredField`)) : true
      }}]">
