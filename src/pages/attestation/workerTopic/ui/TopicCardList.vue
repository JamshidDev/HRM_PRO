          ...
          <template v-for="(item, idx) in store.list" :key="idx">
            ...
            >
              ...
              <div class="flex flex-col text-xs text-secondary md:w-[260px] w-full">
                <p
                  class="leading-[1.2] line-clamp-1 w-full"
                  :class="[
                    existAttemptCount(item.results, item.chances) === 0 && 'line-through opacity-50'
                  ]"
                >
                  {{ $t('examPage.attemptsCount') }}:
                  <span class="font-semibold text-textColor0">{{
                    existAttemptCount(item.results, item.chances)
                  }}</span>
                </p>
                <p class="leading-[1.2] line-clamp-1">
                  {{ $t('solveExamPage.deadline') }}:
                  <span class="font-semibold text-textColor0">{{ item.deadline }}</span>
                </p>
              </div>
              ...
              <div class="flex flex-col text-xs md:w-[260px] w-full text-secondary">
                ...
                <p class="leading-[1.2] line-clamp-1">
                  {{ $t('solveExamPage.questions') }}:
                  <span class="font-semibold text-textColor0">{{ item.tests_count }}</span>
                </p>
              </div>
              <div class="flex md:w-[120px]! w-full! md:justify-end">
                ...
                <n-button class="w-full! md:w-auto! mt-4! md:mt-0!" v-else secondary size="small">
                  {{ $t('content.finished') }}
                </n-button>
              </div>
            </div>
            <div class="mb-2 rounded-bl-lg rounded-br-lg py-1 transition-all bg-surface-section">
              <n-collapse-transition
                class="w-full px-6 bg-surface-section"
                :show="item.id === selectedRowId"
              >
                <div class="grid grid-cols-12 bg-primary/10 w-full rounded-lg py-2 px-4">
                  <template v-for="result in item.results" :key="result.id">
                    <div
                      class="col-span-12 flex items-center my-2 pb-1 border-b border-primary/60 border-dashed"
                    >
                      ...
                      <div class="flex flex-col pl-4 w-[calc(100%-180px)]">
                        <p class="leading-[1.2] text-xs text-primary font-semibold">
                          <span class="text-secondary w-[120px] inline-block"
                            >{{ $t('examPage.startTime') }}:</span
                          >
                          {{ result.created }}
                        </p>
                        <p
                          v-if="result.ended"
                          class="leading-[1.2] text-xs text-primary font-semibold"
                        >
                          <span class="text-secondary w-[120px] inline-block"
                            >{{ $t('examPage.endTime') }}:</span
                          >
                          {{ result.ended }}
                        </p>
                        <p v-else class="text-warning leading-[1.2] text-xs">
                  ...
                  <template v-if="item.results.length === 0">
                    <p class="col-span-12 text-center py-4! text-secondary">
                      {{ $t('content.no-data') }}
                    </p>
                  </template>
                </div>
