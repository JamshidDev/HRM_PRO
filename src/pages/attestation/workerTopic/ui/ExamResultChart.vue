<script setup>
  ...
  use([LegendComponent, TooltipComponent, TitleComponent, PieChart, GridComponent, CanvasRenderer])
  ...
  const colors = ['#E53835', '#7440FD', '#FE9D1D', '#0ded6f']
  ...
  watch(
    ...
    (v) => {
      ...
      let unique = Array.from(
        new Set(
          newData.sort((a, b) => Number(a.count) - Number(b.count)).map((i) => Number(i.count))
        )
      )
