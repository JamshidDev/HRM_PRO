    ...
    >
      <div class="flex gap-2 items-center w-full">
        ...
        <div>
          ...
          <div class="flex flex-col md:flex-row gap-1 md:gap-2 text-xs">
            <p>
              {{ $t('examPage.question_count') }}:
              <span class="font-medium">{{ exam.questions_count }}</span>
            </p>
            <p>
              {{ $t('solveExamPage.variant') }}: <span class="font-medium">{{ exam.variant }}</span>
            </p>
            <p>
              {{ $t('solveExamPage.deadline') }}:
              <span class="font-medium">{{ Utils.timeWithMonth(exam.deadline) }}</span>
            </p>
            <p>
              {{ $t('examPage.attempts') }}: <span class="font-medium">{{ exam.chances }}</span>
            </p>
          </div>
        </div>
      </div>
      <n-tooltip>
        ...
        <template #trigger>
          <n-button
            ...
            :disabled="
              (exam.results.findIndex((i) => typeof i.result != 'number') !== -1 &&
                !examStore.exam_storage?.[exam.id]) ||
              exam.results.length >= exam.chances
            "
            @click="$emit('start')"
          >
            <template #icon>
    ...
    <n-collapse-transition :show="show">
      <div class="px-3">
        ...
        <div class="flex flex-col gap-5">
          <div
            v-for="(item, idx) in exam.results"
            :key="idx"
            class="flex items-center justify-between p-2 bg-surface-ground rounded-md"
            style="box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px"
          >
            <div class="flex gap-2 items-center">
              <n-button size="small" type="primary" secondary class="rounded-xl!"
                ><span class="min-w-[30px]">{{ item?.result || 0 }}</span></n-button
              >
              <div>
                <p class="font-bold">
                  {{ Utils.timeWithMonth(item.created) }} -
                  {{ item.ended ? Utils.timeWithMonth(item.ended) : '...' }}
                </p>
                <div class="inline-block">
                  <n-button size="tiny" secondary :type="item?.ended ? 'primary' : 'warning'">
                    {{ item?.ended ? $t('examPage.finished') : $t('examPage.inProgress') }}
                  </n-button>
                </div>
              </div>
            </div>
