  ...
  <div class="grid grid-cols-12">
    <template v-if="store.selectedFile && !store.excelPreviewLoading">
                ...
                <template v-for="(_, idx) in store.headerRow" :key="idx">
                  <th class="text-center! min-w-[200px]">
                    <div class="flex items-center gap-2">
                      <n-checkbox
                        @click="onChange(store.headerRow[idx].key)"
                        :checked="store.selectedCol === store.headerRow[idx].key"
                      >
                      </n-checkbox>
                      <n-select
      ...
      <div class="col-span-12 flex justify-between mt-4">
        <div
          @click="onBack"
          class="flex cursor-pointer gap-2 rounded-2xl items-center border border-surface-line px-2 py-1"
        >
        ...
        <div class="flex gap-4 items-center">
          <n-button @click="onBack" type="error" secondary
            >{{ $t('content.back') }} <template #icon><ChevronCircleLeft24Regular /></template
          ></n-button>
          <n-button :loading="store.importLoading" @click="onSave" type="primary">{{
            $t('content.save')
          }}</n-button>
        </div>
      </div>
    ...
    <div class="col-span-12" v-else>
      <n-spin :show="store.excelPreviewLoading">
        <div
          @click="clickEv"
          class="w-full bg-patter h-[120px] border border-surface-line border-dashed rounded-2xl mb-4 flex justify-center items-center cursor-pointer hover:scale-[0.96] scale-[1] transition-all duration-300"
        >
          <span class="font-semibold text-success flex gap-2 text-xl items-center"
            ><n-icon size="42"><CloudCheckmark24Regular /></n-icon>
            {{ $t('categoryPage.pressUpload') }}</span
          >
  ...
  .bg-patter {
    ...
    background:
      ...
      radial-gradient(
        ...
        25px 25px,
      linear-gradient(#ecedf4 2px, transparent 2px) 0 -1px,
      linear-gradient(90deg, #ecedf4 2px, #ffffff 2px) -1px 0;
    background-size:
      50px 50px,
      50px 50px,
      25px 25px,
