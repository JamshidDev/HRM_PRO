...
<template>
  <div
    class="border bg-primary/5 rounded-lg overflow-hidden border-primary/20 p-2 shadow-primary/5 drop-shadow-xs flex flex-col gap-3"
  >
    <UIEditorViewer :html="question.question"></UIEditorViewer>
    <template v-for="(option, idx) in question.answers" :key="idx">
      <div
        :class="[option.id === question.result && 'bg-danger/20']"
        class="flex gap-2 p-2 items-center border rounded-md border-primary/10"
      >
        <div class="relative w-4! h-4!">
          <div class="radio" :class="{ 'radio-error': option.id === question.result }" />
        </div>
