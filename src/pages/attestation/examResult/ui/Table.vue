...
<template>
  <div class="w-full overflow-x-auto">
    <n-spin :show="store.loading || attemptStore.loading">
      <n-table v-if="store.list.length > 0" size="small" class="w-full mt-4" :single-line="false">
        <thead>
          <tr>
        ...
        <tbody>
          ...
          >
            <td class="text-center!">
              {{ idx + 1 + (store.params.page - 1) * store.params.per_page }}
            </td>
            <td>
              <UIUser
                :hide-tooltip="true"
            ...
            <td>
              <n-tooltip trigger="hover">
                <template #trigger>
                  <p
                    class="w-[200px] line-clamp-1 border rounded-lg border-surface-line px-1 bg-dark/4 cursor-pointer"
                  >
                    {{ result.topic?.name }}
                  </p>
                </template>
            ...
            <td class="text-center">
              <template v-if="typeof result?.result === 'number'">
                <n-tag
                  ...
                  :class="resultClass(result.exam?.tests_count, result.result)"
                  round
                >
                  {{ result?.result }}
                </n-tag>
