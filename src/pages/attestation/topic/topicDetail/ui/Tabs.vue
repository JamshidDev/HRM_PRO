    ...
    <div class="shrink-0">
      ...
      <div class="grid grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-2 my-4">
        <div
          ...
          class="w-full p-4 py-3 bg-blue-50 rounded-xl transition-all bg-gradient-card-1 hover:bg-blue-100 cursor-pointer flex flex-col gap-2"
          :class="{ 'bg-info/10!': activeTab === examTabNumber }"
        >
          <div class="flex items-center gap-1">
            <n-icon
              :border-radius="6"
              :color="TopicUtils.getMediaProperty(examTabNumber).color"
              :size="30"
              class="shrink-0"
            >
        ...
        <template v-for="(item, idx) in fileStore.list">
          ...
          >
            <div class="flex items-center gap-1">
              <n-icon
                :border-radius="6"
                :color="TopicUtils.getMediaProperty(item.id).color"
                :size="30"
                class="shrink-0"
              >
                <n-icon
