...
<template>
  <div
    class="flex justify-center items-center h-screen bg-no-repeat bg-cover bg-center bg-gradient--v1 relative"
  >
    ...
    <div
      class="bg-surface-section relative z-10 rounded-xl flex justify-between xl:p-[20px] border border-surface-line bg-[url('/wing.png')] bg-bottom bg-cover"
    >
      <n-form
        ref="formRef"
      ...
      >
        ...
        <span class="text-center text-sm mb-[20px] text-gray-500">{{
        ...
        <div class="grid">
          <n-button size="large" type="primary" :loading="store.loading" @click="onSubmit"
            >{{ $t(`loginPage.login`) }}
          </n-button>
          <template v-if="appStore.appConfig.signatureLogin">
            <n-divider title-placement="center">{{ $t('content.or') }}</n-divider>
            <n-button size="large" type="warning" @click="onSignatureLogin"
