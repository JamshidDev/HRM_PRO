  ...
  <div class="w-full">
    <div class="flex justify-center items-center w-full h-[100px] relative">
      <transition-group name="list" tag="div" class="flex justify-center items-center relative">
        <template v-for="(user, index) in allowedShowUsers" :key="user.id">
          <div class="relative group">
            ...
            <div
              class="pointer-events-none absolute bottom-[10px] group-hover:bottom-[-50px] left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-[#6f99ff] text-white text-xs rounded opacity-0 group-hover:opacity-100 whitespace-nowrap transition-all duration-250 line-clamp-1"
            >
              {{ user.short_name }}
            </div>
          </div>
        ...
        <transition name="count-fade" :key="`count-${othersUserCount}`">
          <div
            ...
            @click="store.userVisible = true"
            class="ml-2 text-[#6f99ff] font-bold text-lg cursor-pointer"
          >
            +{{ othersUserCount }}
          </div>
        </transition>
    ...
    <h3
      ...
      :style="{
        backgroundImage:
          'linear-gradient(135deg, rgb(59, 130, 246) 0px, rgb(139, 92, 246) 50%, rgb(6, 182, 212) 100%)',
        backgroundClip: 'text',
