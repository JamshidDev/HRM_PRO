<template>
  ...
  >
    ...
    >
      ...
      <span @click="emits('onReaction', item.emoji)" class="relative scale-100 select-none flex transform transition-all duration-300 group-hover/box:scale-160">
        {{ item.emoji }}
      </span>
...
@keyframes flyUp {
  ...
  30% {
    opacity: 1;
    transform: translate(calc(-50% + var(--fly-x) * 0.3), calc(-50% - 50px)) scale(1.3) rotate(calc(var(--rotation) * 0.5));
  }
  100% {
    opacity: 0;
    transform: translate(calc(-50% + var(--fly-x)), calc(-50% - 140px)) scale(0.3) rotate(var(--rotation));
  }
}
</style>