    ...
    <div class="grid grid-cols-12 gap-x-4">
      <div class="col-span-12 mb-10">
        ...
        <p class="text-lg text-primary">
          {{
            store.account?.worker?.last_name +
            ' ' +
            store.account?.worker.first_name +
            ' ' +
            store.account?.worker.middle_name
          }}
        </p>
      ...
      <n-form-item class="col-span-6" :label="$t(`profilePage.form.confirmPassword`)">
        <n-input type="text" v-model:value="store.payload.password" />
      </n-form-item>
    </div>
