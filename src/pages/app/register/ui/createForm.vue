...
<template>
  ...
  >
    <div class="w-full">
      <span class="text-2xl font-bold text-center block uppercase">{{
    ...
    <span class="text-xs mt-3"
      >{{ $t('registerPage.doYouHaveAccount') }}
      <span class="text-primary" @click="router.push(AppPaths.Login)">{{
        $t('registerPage.loginToSystem')
      }}</span>
    </span>
  </n-form>
</template>
