...
<template>
  ...
  >
    ...
    <div class="w-full my-5">
      <span class="text-2xl font-bold text-center block uppercase text-primary">{{
        $t(`registerPage.subtitle`)
      }}</span>
      <span class="text-xs text-center block text-gray-500">{{
        $t(`registerPage.verification`, { n: Utils.maskText(store.payload.phone || '', 0, 4) })
      }}</span>
    </div>
    <div
