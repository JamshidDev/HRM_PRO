...
const onSearchEv = () => {
  ...
  if(!Boolean(query)){
    ...
    store.allPermissionList = store.originAllPermissionList.filter(v=>v?.name?.toString()?.toLowerCase()?.includes(query))
  }
}
      ...
      <n-form-item :label="$t(`userRole.form.name`)" path="name">
        ...
        <n-form-item :label="$t(`userRole.form.permissions`)" path="permissions">
          <div class="w-full">
            ...
            <div class="w-full h-[calc(100vh-300px)] overflow-y-auto border border-surface-line rounded-lg p-2">
