...
<template>
  <n-form ref="formRef" :rules="validationRules.districtPage" :model="store.payload">
    <div style="min-height: calc(100vh - 120px)">
      ...
      <n-form-item :label="$t(`districtPage.form.name`)" path="name" :rule-path="validationRules.rulesNames.requiredStringField">
        <UIMultipleLangItems>
          <template #uz-content>
      ...
      <n-form-item :label="$t(`districtPage.form.coordinates`)">
        <Map
          :default-center="
            store.visibleType ? appStore.mapLocation.center : store.payload.marker.coords
          "
