  ...
  <UIPageContent>
    ...
    <UIDConfirm v-model:visible="store.confirmVisible">
      ...
      <h2
        ...
        :class="[store.isSpam ? 'text-success' : 'text-danger']"
      >
        {{ $t(store.isSpam ? 'userPage.form.cancelSpam' : 'userPage.form.confirmSpam') }}
      </h2>
      <template #action>
        <div class="grid grid-cols-2 gap-2">
          <n-button @click="store.confirmVisible = false" type="error" secondary>{{
            $t('content.close')
          }}</n-button>
          <n-button @click="store._onSpam" type="primary" secondary>{{
            $t('content.confirm')
          }}</n-button>
        </div>
      </template>
