      ...
      <n-table class="mt-10" :single-line="false" size="small">
        ...
        <tbody>
          <tr v-for="(item, idx) in store.authList" :key="idx">
            ...
            <td>
              <UIUser
                :data="{
                  ...
                  position: item?.user?.worker?.phone
                }"
              />
            </td>
            <td>
              <div class="flex items-center justify-between">
                ...
                <n-icon
                  v-if="store.authActiveCol === item.id"
                  @click="openCode(null)"
                  size="24"
              ...
              <n-collapse-transition
                class="text-xs bg-dark text-white rounded-xl shadow w-full overflow-y-auto p-2"
                :show="store.authActiveCol === item.id"
              >
                <pre>
                {{ item }}
              </pre
            ...
            <td>
              <div class="w-full text-center">{{ Utils.timeWithMonth(item?.login_at) }}</div>
            </td>
            <td>
              <div class="w-full text-center text-sm">
                {{ item?.logout_at ? Utils.timeWithMonth(item?.logout_at) : '' }}
              </div>
            </td>
            <td>
              <div class="w-full text-center uppercase font-medium">{{ item?.ip_address }}</div>
            </td>
          </tr>
        </tbody>
      </n-table>
