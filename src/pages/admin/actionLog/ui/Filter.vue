  ...
  const filterCount = computed(() => {
    if (store.activeTab === 1) {
      return (
        ...
        Number(Boolean(store.params.created_at)) +
        Number(Boolean(store.params.subject_type)) +
        Number(Boolean(store.params.description))
      )
    } else {
      return (
        Number(Boolean(store.params.organizations.length)) +
        Number(Boolean(store.params.login_at)) +
  ...
  >
    ...
    <template #filterContent>
      ...
      <template v-if="store.activeTab === 1">
        ...
        <n-date-picker
          clearable
          v-model:value="store.params.created_at"
          type="date"
      ...
      <template v-if="store.activeTab === 2">
        ...
        <n-date-picker
          clearable
          v-model:value="store.params.login_at"
          type="date"
          @update:value="filterEvent"
        />
      </template>
