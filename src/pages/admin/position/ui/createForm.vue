...
<template>
  <n-form ref="formRef" :rules="validationRules.common" :model="store.payload">
    <div style="min-height: calc(100vh - 120px)">
      <n-form-item :label="$t(`positionPage.form.name`)" path="name" :rule-path="validationRules.rulesNames.requiredStringField">
        <UIMultipleLangItems>
          <template #uz-content>
      ...
      <n-form-item
        :label="$t(`positionPage.form.classification_index`)"
        path="classification_index"
      >
        <n-input type="text" v-model:value="store.payload.classification_index" />
      </n-form-item>
      <n-form-item :label="$t(`positionPage.form.classification_code`)" path="classification_code">
        <n-input type="text" v-model:value="store.payload.classification_code" />
      </n-form-item>
    </div>
