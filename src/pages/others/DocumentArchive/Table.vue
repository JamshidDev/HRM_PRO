  ...
  <n-spin :show="store.loading">
    <div class="w-full" v-if="store.list.length > 0">
      <template v-for="(item, idx) in store.list" :key="idx">
        <div
          class="w-full max-w-[1200px] mx-auto flex flex-col md:flex-row p-4 border border-gray-200 mb-2 cursor-pointer rounded-lg bg-surface-section hover:shadow-sm transition-all"
        >
          <div class="flex flex-col" style="width: calc(100% - 80px)">
          ...
          <div class="flex flex-col w-full md:w-[80px]">
            <n-icon size="42" class="mx-auto mt-6 text-success" @click="downloadFile(item.file)">
              <ArrowCircleDown12Regular />
            </n-icon>
            <span class="text-center text-success" @click="downloadFile(item.file)">{{
              $t('content.download')
            }}</span>
          </div>
        </div>
