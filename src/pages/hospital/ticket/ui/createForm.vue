  ...
  const onSubmit = () => {
    formRef.value?.validate((error) => {
      if (!error) {
        ...
        store.payload.worker_id = componentStore.workerList.filter(
    ...
    <n-form ref="formRef" :rules="validationRules.common" :model="store.payload">
      ...
      <div class="w-full h-[calc(100vh-320px)] overflow-y-auto">
        <template v-for="(item, index) in store.selectedWorkers" :key="index">
          <div
            class="flex items-center justify-between bg-surface/4 hover:bg-surface/8 mb-1 py-1 px-2 rounded-lg"
          >
            <UIUser
              :short="false"
              :data="{
            ...
            />
            <n-button @click="store._removeEv(item)" type="error" circle secondary size="small">
              <template #icon>
        ...
        <template v-if="store.selectedWorkers.length === 0">
          <span class="text-center text-danger/70 text-xs mt-4 inline-block w-full">{{
            $t('ticket.form.no-data')
          }}</span>
        </template>
      </div>
    </n-form>
