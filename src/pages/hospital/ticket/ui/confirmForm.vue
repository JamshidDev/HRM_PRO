...
<template>
  <div class="overflow-y-auto">
    <n-form ref="formRef" :rules="validationRules.common" :model="store.confirmPayload">
      <n-form-item
      ...
      <n-form-item :label="$t(`medPage.form.comment`)" path="comment">
        <n-input rows="2" type="textarea" v-model:value="store.confirmPayload.comment" />
      </n-form-item>
    </n-form>
  </div>
  <div class="grid grid-cols-2 gap-2">
    ...
    <n-button @click="onSubmit" :loading="store.confirmLoading" type="primary">
      {{ $t('content.save') }}
    </n-button>
  </div>
</template>
