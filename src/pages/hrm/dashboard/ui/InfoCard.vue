...
<template>
  <div
    class="h-full min-h-[400px] flex flex-col gap-1 md:gap-3"
    v-if="store.dashboard?.passwordCard"
  >
    <div
      class="grow p-4 basis-auto border-surface-line flex flex-col rounded-lg bg-surface-section hover-effect-card items-start justify-between"
    >
      <div class="flex justify-between w-full items-center">
        <n-button text type="primary">
          <span class="font-semibold text-nowrap text-textColor0"
            >{{ $t(store.dashboard?.passwordCard?.title) }}
          </span>
          <template #icon>
            <n-icon>
              <ContactCardRibbon16Regular />
        ...
        <p
          @click="$emit('detail', InfoCardEnum.PASSPORT)"
    ...
    >
      <div class="flex justify-between w-full items-center">
        <n-button text type="success">
          <span class="font-semibold text-nowrap text-textColor0">{{
            $t(store.dashboard?.medicalCard?.title)
          }}</span>
          <template #icon>
            <n-icon>
              <BriefcaseMedical20Regular />
        ...
        <!--       @click="$emit('detail')"-->
        <p
          @click="$emit('detail', InfoCardEnum.MED)"
          aria-disabled="true"
          class="leading-[14px] text-primary border-b border-dashed border-primary cursor-pointer transition-all hover:border-primary/80 hover:text-primary/80"
        >
          {{ $t('content.view') }}
    ...
    >
      <div class="flex justify-between w-full items-center">
        <n-button text type="error">
          <span class="font-semibold text-nowrap text-textColor0"
            >{{ $t(store.dashboard?.pensionCard?.title) }}
          </span>
          <template #icon>
            <n-icon>
              <PeopleToolbox20Regular />
        ...
        <p
          @click="$emit('detail', InfoCardEnum.PENSION)"
          class="leading-[14px] text-primary border-b border-dashed border-primary cursor-pointer transition-all hover:border-primary/80 hover:text-primary/80"
        >
          {{ $t('content.view') }}
        </p>
