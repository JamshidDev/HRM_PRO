...
<template>
  ...
  >
    ...
    ></span>
    <span
      class="z-1 opacity-30 absolute bottom-0 left-0 w-[160px] h-[200px] rotate-180 bg-no-repeat bg-[url(/effect/primary-card.svg)]"
    ></span>
    <div class="flex items-center gap-2 mb-10">
      <span class="font-semibold text-nowrap text-textColor2">{{
        $t('dashboardPage.card.vacation')
      }}</span>
    ...
    <!--    </div>-->
    <div
      v-for="(item, idx) in store.dashboard.vacations"
      class="w-full flex mb-3 items-center cursor-pointer text-textColor0"
    >
      <div
      ...
      <div class="font-bold w-[60px] text-end">{{ item.active_vacations }}</div>
    </div>
  </div>
</template>
