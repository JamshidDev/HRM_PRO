...
<template>
  ...
  >
    <span
      class="z-0 opacity-40 absolute top-0 right-0 w-[160px] h-full bg-no-repeat bg-[url(/effect/effect-card-2.svg)]"
    ></span>
    <template v-if="store.dashboard.birthdays">
      <div class="flex justify-between items-center mb-4">
        <p class="font-semibold text-lg text-textColor0">
          {{ $t(store.dashboard.birthdays.title) }}
        </p>
        <p
          @click="$emit('detail')"
      ...
      <template v-for="(item, idx) in store.dashboard.birthdays.data" :key="idx">
        <div class="flex justify-between border-b border-surface-line items-center h-[50px]">
          ...
          <span class="text-textColor1 text-xs" v-if="item.workers.length === 0">
            <n-button secondary size="tiny" class="text-secondary!">{{
              $t('content.noAvailable')
            }}</n-button>
          </span>
        </div>
      </template>
