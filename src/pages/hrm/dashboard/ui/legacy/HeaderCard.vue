...
<template>
  <div
    class="w-full border border-surface-line p-4 rounded-lg bg-surface-section relative cursor-pointer"
  >
    <span
    ...
    <div class="w-full flex justify-between mt-4">
      <span class="text-xs text-textColor0"
        >{{ $t(card?.data1.title) }} -
        <span class="text-sm font-semibold">{{ card?.data1?.count }}</span></span
      >
      <span class="text-xs text-textColor2"
        >{{ $t(card?.data2.title) }} -
        <span class="text-sm font-semibold">{{ card?.data2?.count }}</span></span
      >
    </div>
  </div>
</template>
