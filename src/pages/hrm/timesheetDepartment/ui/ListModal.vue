...
<template>
  <UIModal
      v-model:visible="store.listVisible"
      :width="600"
    ...
    <div class="w-full overflow-y-auto  h-[600px] border border-surface-line rounded-lg p-2 ">
      <template v-for="item in store.previewList">
        <div class="mb-3 flex bg-surface-ground p-1 rounded-lg items-center">
          <div class="w-[calc(100%-40px)] px-2">
            ...
            <span class="line-clamp-1 text-secondary text-xs leading-[1.2]">{{item?.organization?.name}}</span>
          </div>
          <n-button :loading="store.deleteLoading" @click="onDelete(item)" type="error" secondary size="small">
            <template #icon>
              <Delete20Filled/>
            </template>
          </n-button>
      ...
      <p v-if="store.previewList.length === 0" class="text-center w-full text-secondary !mt-[100px]">{{$t('content.no-data')}}</p>
