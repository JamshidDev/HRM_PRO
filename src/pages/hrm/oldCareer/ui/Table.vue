<script setup>
  ...
  const onUpdate = (v) => {
    const data = store.list.map((v, index) => ({
      worker_old_career_id: v.id,
      position: index
    }))
...
<template>
  <div>
    <n-spin :show="store.newCareerLoading">
      ...
      >
        <span class="text-lg font-medium" v-if="store.newCareers.length > 0">{{
      ...
      <div class="w-full overflow-x-auto" v-if="store.newCareers.length > 0">
        <n-table class="select-none" :single-line="false" size="small">
          <thead>
            <tr>
          ...
          <tbody class="sort-target">
            <tr v-for="(item, idx) in store.newCareers" :key="idx">
              <td>
    ...
    <n-spin :show="store.loading">
      ...
      <div class="w-full overflow-x-auto" v-if="store.list.length > 0">
        <VueDraggable
          ...
          @end="onUpdate"
        >
          <n-table class="mt-4 select-none" :single-line="false" size="small">
            <thead>
              <tr>
                <th class="text-center! min-w-[40px] w-[40px] handle">
                  {{ $t('content.number') }}
                </th>
            ...
            <tbody class="sort-target">
              <tr v-for="(item, idx) in store.list" :key="idx">
                ...
                <td>
                  <UIMenuButton :data="item" :show-edit="true" @selectEv="selectEv" />
                </td>
              </tr>
            </tbody>
          </n-table>
