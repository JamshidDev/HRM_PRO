...
<template>
  <n-spin :content-style="{ width: '100%' }" :show="store.loading">
    ...
    >
      <div
        class="p-2 border-b md:border-b-0 md:border-r border-surface-line basis-auto md:basis-[70%] shrink-0 grow overflow-auto"
      >
        <div class="w-full h-full overflow-auto">
          <VueDraggable
            v-if="store.payload.columns.length"
            data-id-attr="column"
            :animation="150"
            ...
            <n-table :single-line="false" size="small" class="select-none">
              <thead>
                <tr class="sort-target">
                  <th
                    class="cursor-move !bg-surface-ground"
                    v-for="(header, idx) in store.payload.columns"
                    :key="header.column"
                  >
              ...
              <tbody>
                <tr v-for="i in 3" :key="i">
                  <td v-for="header in store.payload.columns" :key="header.column">&nbsp;</td>
                </tr>
              </tbody>
            </n-table>
          </VueDraggable>
      ...
      <div class="p-2 shrink-0 md:h-full overflow-y-auto basis-auto overflow-x-hidden">
        <ul class="flex md:block gap-2 flex-wrap w-full">
          ...
          <li
            v-for="(item, idx) in store.columns"
