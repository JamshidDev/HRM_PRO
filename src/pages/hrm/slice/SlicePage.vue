<script setup>
  ...
  import { useReportStore, useAccountStore } from '@/store/modules/index.js'
  ...
  onMounted(() => {
    if (!accStore.checkAction(accStore.pn.hrJobsRead)) return
  ...
  <div class="p-2">
    <SliceContainer>
      <splitpanes class="w-full">
        <template v-for="(panel, idx) in store.cols" :key="panel.id">
          <pane min-size="10" v-if="store.isActivePanel(panel.id)">
            <SectionPanel :data="panel">
