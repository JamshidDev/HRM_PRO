...
<template>
  <n-spin class="min-w-[600px]" :show="store.positionLoading" style="min-height: 200px">
    ...
    <div class="w-full" v-if="store.positionList.length > 0">
      <n-table class="mt-4" size="small" :single-column="true" :striped="true">
        <thead>
          <tr>
          ...
          >
            <td>
              <span class="text-[12px] text-gray-600 block">{{
                (store.positionParams.page - 1) * store.positionParams.per_page + idx + 1
              }}</span>
            </td>
            <td>
              <n-checkbox :checked="store.position === item.id"></n-checkbox>
            </td>
