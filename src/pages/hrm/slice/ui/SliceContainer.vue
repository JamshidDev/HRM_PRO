...
<template>
  ...
  >
    <div class="w-full flex justify-between px-2 py-1 border-b border-surface-line mb-2">
      <div class="flex items-center gap-2">
        <n-breadcrumb>
          <template v-for="item in store.barList.sort((a, b) => a.id - b.id)" :key="item.id">
            <n-breadcrumb-item class="cursor-pointer" @click="store.addPanel(item)">
      ...
      <n-button @click="store.changeScreenMode()" circle size="small">
        <template #icon>
    ...
    <TransitionGroup tag="div" name="grid-fade" class="w-full px-2">
