<script setup>
  ...
  const onChangeDate = (ts, index) => {
    ...
    store.worker.list[index].period_to = ts
      ? new Date(ts).setFullYear(date.getFullYear() + 1)
      : null
  }
...
<template>
  ...
  >
    <div
      class="col-span-12 grid grid-cols-12 gap-x-4 bg-surface-ground p-2 !pb-0 border border-surface-line rounded-xl mb-4 border-dashed"
    >
      <n-form-item
        class="col-span-2"
    ...
    >
      <template #filterAction>
        <n-button
          type="primary"
          @click="store._fetchAutoGenerate()"
    ...
    <n-spin class="col-span-12 mt-2" :show="store.worker.loading">
      <div
        class="relative border border-surface-line rounded-lg h-[calc(100vh-340px)] overflow-y-auto mb-4"
      >
        <div
          class="grid grid-cols-12 border-b border-surface-line gap-1 py-2 text-xs font-semibold text-secondary sticky top-0 left-0 right-0 w-full bg-surface-section z-10"
        >
        ...
        <template v-for="(item, idx) in store.worker.list" :key="item">
          <div
            class="grid mx-2 grid-cols-12 gap-1 border border-surface-line bg-surface-ground/80 px-2 py-2 mb-4 rounded-2xl"
          >
            ...
            <n-form-item
              ...
              :show-label="false"
            >
              <n-input class="w-full" type="text" v-model:value="item.table_number" />
            </n-form-item>
            <n-form-item
              class="col-span-2"
            ...
            <n-form-item
              ...
              :show-label="false"
            >
              <n-input-number class="w-full" min="0" max="100" v-model:value="item.all_days" />
            </n-form-item>
          </div>
        </template>
      </div>
