    ...
    <div style="min-height: calc(100vh - 120px)">
        ...
        :rule="[
          {
            trigger: ['blur-sm'],
            validator() {
              if (store.payload.phones.find((i) => i.length !== 5 && i.length !== 9) !== undefined)
                return new Error($t('organizationLeaderPage.invalidPhone'))
              return true
            }
          }
        ]"
      >
        <div class="flex gap-2 flex-wrap">
          <template v-for="(item, idx) in store.payload.phones" :key="idx">
            <div class="grow shrink-0 basis-[200px]">
              <PhoneInput
                ...
                :addable="
                  store.payload.phones.length < 5 && idx === store.payload.phones.length - 1
                "
