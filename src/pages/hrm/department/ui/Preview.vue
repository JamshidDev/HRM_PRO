      ...
      <n-spin :show="store.previewLoading" class="h-full">
        <template v-for="(item, index) in store.previewList" :key="item.id">
          <div class="flex items-center bg-surface-ground px-2 py-1 rounded-xl mb-2 border border-surface-line">
            ...
            >
              <template #position>
                <div class="text-xs text-secondary"><n-button secondary type="primary" size="tiny" class="!h-[14px]"><span class="text-primary font-medium">{{$t('documentPage.form.rate')}} - {{item.rate}}</span></n-button> {{item?.department?.name}}</div>
              </template>
            </UIUser>
          </div>
        </template>
