    ...
    series: [
        {
            ...
            data: [
                { value: 0, name: t('dashboardPage.rewardCard.reward.shortTypeOne'), itemStyle: { color: '#1A84FF' } },
                { value: 0, name: t('dashboardPage.rewardCard.reward.shortTypeTwo'), itemStyle: { color: '#2dcb73' } },
            ]
        }
    ]
}
    ...
    series: [
        {
            ...
            data: [
                { value: 0, name: t('dashboardPage.rewardCard.punishment.shortTypeOne'), itemStyle: { color: '#1A84FF' } },
                { value: 0, name: t('dashboardPage.rewardCard.punishment.shortTypeTwo'), itemStyle: { color: '#E53835' } },
            ]
        }
    ]
}

watch(() => store.dashboard.incentivesCount,(v)=>{
...
<template>
    <div class="w-full h-full border border-surface-line p-4 rounded-lg bg-surface-section relative hover-effect-card">
      <n-spin :show="store.loadingTwo">
        <span
            class="z-1 opacity-30 absolute top-0 right-0 w-[160px] h-full bg-no-repeat bg-[url(/effect/effect-card-1.svg)]"></span>
        <div class="flex h-auto">
          <div class="text-textColor2" style="width: calc(100% - 280px)">
            <p class="font-semibold text-textColor2">{{ $t('dashboardPage.rewardCard.reward.title') }}</p>
          ...
          <div class="w-[280px] h-[120px] relative z-2">
            <div class="flex justify-end">
              <p @click="$emit('detail', InfoCardEnum.INCENTIVE)" aria-disabled="true" class="leading-[14px] text-primary border-b border-dashed border-primary  cursor-pointer transition-all hover:border-primary/80 hover:text-primary/80">{{$t("content.view")}}</p>
            </div>
        ...
        <div class="flex h-auto mt-2">
          <div class="text-textColor2" style="width: calc(100% - 280px)">
            <p class="font-semibold text-textColor2">{{ $t('dashboardPage.rewardCard.punishment.title') }}
            </p>
          ...
          <div class="w-[280px] h-[120px] relative z-2">
            <div class="flex justify-end">
              <p @click="$emit('detail', InfoCardEnum.DISCIPLINARY)" aria-disabled="true" class="leading-[14px] text-primary border-b border-dashed border-primary  cursor-pointer transition-all hover:border-primary/80 hover:text-primary/80">{{$t("content.view")}}</p>
            </div>
