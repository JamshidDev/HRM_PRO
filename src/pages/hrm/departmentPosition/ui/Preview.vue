...
<template>
  <UIModal width="700px" v-model:visible="store.preview.visible" :title="$t('content.view')">
    <div class="w-full h-[calc(100vh-360px)]">
      <n-spin :show="store.preview.loading" class="h-full">
        <template v-for="(item, index) in store.preview.list" :key="item.id">
          <div
            class="flex items-center bg-surface-ground px-2 py-1 rounded-xl mb-2 border border-surface-line"
          >
                ...
                <div class="text-xs text-secondary line-clamp-1">
                  <n-button secondary type="primary" size="tiny" class="!h-[14px]"
                    ><span class="text-primary font-medium"
                      >{{ $t('documentPage.form.rate') }} - {{ item.rate }}</span
                    ></n-button
                  >
                  {{ item?.position?.name }}
                </div>
