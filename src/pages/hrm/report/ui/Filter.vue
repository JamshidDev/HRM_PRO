        ...
        <template #label="{data}">
          <div class="flex items-center justify-between w-full pl-1 pt-1">
            ...
            <div class="shrink-0 w-[80px] grid grid-cols-2 gap-1 pr-2">
              <n-button :type="data.rate>data.real_rate? 'success' :  'default'" class="!text-[10px]" size="tiny" secondary>{{data.rate}}</n-button>
              <n-button  :type="data.real_rate>data.rate? 'error' :  'default'" class="!text-[10px]" size="tiny" secondary>{{data.real_rate}}</n-button>
            </div>
          </div>
