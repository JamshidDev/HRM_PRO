...
<template>
  <UIPageContent>
    ...
    <n-form class="w-full" ref="formRef" :rules="validationRules.common" :model="store.payload">
      <n-steps vertical>
        <n-step :title="$t('createWorkerPage.step.personal')">
          <template #default>
            <PersonalForm ref="personalFormRef" />
          </template>
        </n-step>
        <n-step :title="$t('createWorkerPage.step.pasport')">
          <template #default>
            <PassportFrom />
          </template>
        </n-step>
        <n-step :title="$t('createWorkerPage.step.photo')">
          <template #default>
            <PhotoForm
    ...
    <div class="col-span-12 flex justify-end">
      <n-button type="primary" icon-placement="right" @click="save" :loading="store.saveLoading">
        <template #icon>
          <n-icon>
            <Save16Regular />
          </n-icon>
