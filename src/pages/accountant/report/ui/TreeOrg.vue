      ...
      <n-table class="!border-t-0 sticky-table-header" :single-line="false" size="small">
        ...
        <tbody>
          <template v-for="(item, idx) in flattenData" :key="idx">
            <tr class="hover-row" :class="[item.id === store.params.organization_id && 'selectedRow']">
              <td @click="store.onChangeStructure(item)">
                <n-checkbox :checked="item.id === store.params.organization_id"></n-checkbox>
              </td>
              <td @dblclick="store.onChangeStructure(item)"
                ...
                <div :style="{ paddingLeft: (item.level * 15 + 'px') }" class="flex items-start">
                  <div class="flex justify-end w-[40px] cursor-pointer">
                    <n-icon v-if="item.isHasChildren" @click="toggleExpand(item.id)"
                      :class="[store.expandSet.has(item.id) ? 'rotate-90' : 'rotate-0']" class="transition-all "
                      size="18">
                      <ChevronRight12Regular />
                    </n-icon>
                    <n-icon size="20">
                      <template v-if="item.isHasChildren">
                        <FolderOpen24Filled v-if="!store.expandSet.has(item.id)" class="text-[#a312df]" />
                        <Folder20Filled v-else class="text-[#a312df]" />
                      </template>
                      <DocumentBulletList24Filled v-else class="text-primary" />
                    </n-icon>
                  </div>
                  <span class="ml-2 leading-[1.2] inline-block !text-wrap text-sm w-[calc(100%-40px)]">{{ ' ' +
                    item.name }}</span>
                </div>
              ...
              <td>
                <div class="flex justify-center w-full">
                  <IndicatorStatus :status="item.uploadStats?.[0]?.confirmed"
                    :count="item.uploadStats?.[0]?.uploaded_count" />
                </div>
              </td>
              <td>
                <div class="flex justify-center w-full">
                  <IndicatorStatus :status="item.uploadStats?.[1]?.confirmed"
                    :count="item.uploadStats?.[1]?.uploaded_count" />
                </div>
              </td>
              <td>
                <div class="flex justify-center w-full">
                  <IndicatorStatus :status="item.uploadStats?.[2]?.confirmed"
                    :count="item.uploadStats?.[2]?.uploaded_count" />
                </div>
              </td>
              <td>
                <div class="flex justify-center w-full">
                  <IndicatorStatus :status="item.uploadStats?.[3]?.confirmed"
