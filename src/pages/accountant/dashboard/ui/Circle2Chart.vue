...
<template>
  <div class="flex bg-surface-section p-2 rounded-2xl border border-surface-line h-full">
    ...
    <div class="w-[calc(100%-200px)] pl-2 grid grid-cols-12 content-between min-h-[260px]">
      <div class="col-span-12">
        <template v-for="(item, index) in data">
          <div class="leading-[1] mb-2">
            <p v-bind:style="{ color: colors[item.key] }" class="text-lg font-black mb-0">
              {{ numeral(item.real).format('0,0.0')
              }}<small class="text-xs text-textColor3/60 font-normal pl-1 lowercase">
                {{ $t('content.sum') }}</small
              >
            </p>
            <p class="text-xs text-textColor3 line-clamp-2">
      ...
      <div
        class="border p-1 border-dashed rounded-2xl mt-4 col-span-12"
        :class="[
          isCorrect
      ...
      >
        <div class="leading-[1] flex">
          <p class="text-center font-black text-3xl leading-[1] w-[140px]">
            ...
            <small class="font-normal text-xs text-textColor3/40"> {{ $t('content.mln') }}</small>
          </p>
          <div
            class="text-xs text-textColor3/40 text-center flex items-center leading-[1] w-[calc(100%-140px)]"
          >
            <n-icon size="14" class="rotate-[-20deg] text-textColor3">
              <Line32Filled />
            </n-icon>
            <p class="line-clamp-2">{{ $t('accDashboard.chart.sum') }}</p>
          </div>
        </div>
        ...
        <div class="leading-[1] flex">
          ...
          >
            ...
            <span class="line-clamp-2">{{ $t('accDashboard.chart.total_two') }}</span>
          </div>
        </div>
      </div>
    </div>
